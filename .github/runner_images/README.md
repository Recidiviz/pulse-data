...
Build an image:
```bash
# From runner_images/python311-postgres18 directory, run:
packer init templates/python311-postgres18.pkr.hcl
...
packer build -var "subnet_id=${SUBNET_ID}" templates/python311-postgres18.pkr.hcl
```
### Key Components

- **`scripts/build/provision.sh`**: A provisioning script that installs dependencies, such as Python (via `uv`) and PostgreSQL 18, to prepare the base image.
  
- **`templates/python311-postgres18.pkr.hcl`**: A Packer configuration file that defines the steps necessary to build the custom AMI, including loading user data and running provisioning scripts.
...
### 3. `python311-postgres18.pkr.hcl`
...
```shell script
packer build -var "subnet_id=your-subnet-id" -var "region=us-west-2" templates/python311-postgres18.pkr.hcl
```
...
| `image_name_prefix` | No | `python311-postgres18-runs-on` | Prefix for AMI names (suffixed with architecture and timestamp) |
...
packer build \
  ...
  templates/python311-postgres18.pkr.hcl
```
