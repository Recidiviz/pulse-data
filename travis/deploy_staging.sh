...
gcloud auth activate-service-account recidiviz-staging@appspot.gserviceaccount.com --key-file client-secret-staging.json

# Deploy cron.yaml
gcloud app deploy cron.yaml --project=recidiviz-staging
...
gcloud app deploy queue.yaml --project=recidiviz-staging
...
# Deploy application
gcloud app deploy staging.yaml --project recidiviz-staging --version $VERSION --image-url $IMAGE_URL
