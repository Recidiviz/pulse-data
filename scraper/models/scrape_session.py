...
class ScrapeSession(ndb.Model):
    ...
    Datastore model for information about a scraping session. Used by the scraper
    to store state during scraping, and handle resumption of work from a prior
    session if continuing work after a pause.

    Attributes:
        start: (datetime) Date/time this session started
        end: (datetime) Date/time when this session finished
        docket_item: (string) The task name of the docket item currently leased by this
            session. Used to delete docket items after completion.
...
class ScrapedRecord(ndb.Model):
    ...
    we've already scraped in the session to avoid accidental duplicate scraping
