...
`EntityResolutionDocumentCollectionConfig`, whose generation query the document
store's `build_document_generation_query_builder` builds from this module), so the
standard document upload flow generates, content-addresses, and uploads them like any
other collection.
