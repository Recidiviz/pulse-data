...
def copy_table_to_dataset(
    target_dataset: str, target_table: str, export_query: str, bq_client: BigQueryClient
) -> None:
    ...
    bq_client.create_table_from_query_async(
        target_dataset, target_table, export_query, overwrite=True
    )


def run_export(
    project_id: str, dry_run: bool, state_codes: List[str], target_dataset: str
) -> None:
        ...
        if target_dataset:
            if dry_run:
                ...
                copy_table_to_dataset(
                    target_dataset, table.table_id, export_query, big_query_client
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--target-dataset",
        required=True,
        help="The target BigQuery dataset to export data to",
    )
    ...
    parser.add_argument(
        ...
        required=True,
        nargs="+",
        help="The state codes to export data for",
    )
