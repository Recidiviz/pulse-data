...
# _DAG_GROUP_PACKAGE = initialize_raw_data_dag_group.__name__
...
def _create_test_initialize_raw_data_import_dag() -> None:
    initialize_raw_data_dag_group() >> EmptyOperator(task_id=_DOWNSTREAM_TASK_ID)
...
class TestInitializeRawDataDagGroupIntegration(AirflowIntegrationTest):
    ...
    def test_initializes_dag_secondary_no_state_code(
        self,
    ) -> None:
        test_dag = _create_test_initialize_raw_data_import_dag()
        with Session(bind=self.engine) as session:
            ...
            self.assertEqual(
                ...
                "Must supply [state_code_filter] with a SECONDARY run",
            )
    ...
    def test_successfully_initializes_dag_primary(self) -> None:
        test_dag = _create_test_initialize_raw_data_import_dag()
        with Session(bind=self.engine) as session:
            result = self.run_dag_test(
                dag=test_dag,
