...
def test_create_raw_data_branch_map() -> None:
    ...
    assert len(result) == 4
    assert (
        get_raw_data_import_branch_key(StateCode.US_XX, DirectIngestInstance.PRIMARY)
        in result
    )
    assert (
        get_raw_data_import_branch_key(StateCode.US_XX, DirectIngestInstance.SECONDARY)
        in result
    )
    assert (
        get_raw_data_import_branch_key(StateCode.US_YY, DirectIngestInstance.PRIMARY)
        in result
    )
    assert (
        get_raw_data_import_branch_key(StateCode.US_YY, DirectIngestInstance.SECONDARY)
        in result
    )
