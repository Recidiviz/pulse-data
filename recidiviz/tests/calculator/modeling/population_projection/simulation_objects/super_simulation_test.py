...
def mock_load_table_from_big_query_macro(table_name: str, simulation_tag: str) -> pd.DataFrame:
    return data_dict_macro[table_name][data_dict_macro[table_name].simulation_tag == simulation_tag]
...
def mock_load_table_from_big_query_micro(
    ...
    return data_dict_micro[table_name][data_dict_micro[table_name]['state_code'] == state_code]
...
class TestSuperSimulation(unittest.TestCase):
    ...
    def test_macrosim_data_hydrated(self):
        ...
        self.assertFalse(self.macrosim.initializer.data_dict['total_population_data'].empty)

    @patch('recidiviz.calculator.modeling.population_projection.spark_bq_utils.upload_spark_results',
    ...
    def test_cost_multipliers_multiplicative(self):
        ...
        spending_diff, spending_diff_non_cumulative = outputs['spending_diff'], outputs['spending_diff_non_cumulative']
        outputs_scaled = self.macrosim.upload_simulation_results_to_bq('test', cost_multipliers)
        spending_diff_scaled, spending_diff_non_cumulative_scaled = \
            outputs_scaled['spending_diff'], outputs_scaled['spending_diff_non_cumulative']
        ...
        outputs_doubled = \
            self.macrosim.upload_simulation_results_to_bq('test', partial_cost_multipliers_double)
        spending_diff_double, spending_diff_non_cumulative_double = \
            outputs_doubled['spending_diff'], outputs_doubled['spending_diff_non_cumulative']

        outputs_tripled = \
            self.macrosim.upload_simulation_results_to_bq('test', partial_cost_multipliers_triple)
        spending_diff_triple, spending_diff_non_cumulative_triple = \
            outputs_tripled['spending_diff'], outputs_tripled['spending_diff_non_cumulative']

        assert_frame_equal((spending_diff_triple - spending_diff),
                           (spending_diff_double - spending_diff) * 2)
        assert_frame_equal((spending_diff_non_cumulative_triple - spending_diff_non_cumulative),
    ...
    def test_microsim_data_hydrated(self):
        ...
        self.assertFalse(self.microsim.initializer.data_dict['total_population_data'].empty)
        self.assertFalse(self.microsim.initializer.data_dict['remaining_sentence_data'].empty)
    ...
    def test_using_remaining_sentences_reduces_prison_population(self):
        ...
        affected_time_frame = self.microsim.initializer.data_dict['transitions_data'][
            (self.microsim.initializer.data_dict['transitions_data'].compartment == 'PRISON') &
            (self.microsim.initializer.data_dict['transitions_data'].total_population > 0)].compartment_duration.max()

        # get projected prison population from simulation substituting transitions data for remaining sentences
        substitute_outputs = microsim.simulator.pop_simulations['baseline_middle'].population_projections
        substitute_prison_population = substitute_outputs[
            (substitute_outputs.compartment == 'PRISON')
            & (substitute_outputs.time_step > microsim.initializer.user_inputs['start_time_step'])
            & (substitute_outputs.time_step - microsim.initializer.user_inputs['start_time_step'] < affected_time_frame)
        ].groupby('time_step').sum().total_population

        # get projected prison population from regular simulation
        regular_outputs = self.microsim.validator.pop_simulations['baseline_middle'].population_projections
        regular_prison_population = regular_outputs[
            (regular_outputs.compartment == 'PRISON')
            & (regular_outputs.time_step > self.microsim.initializer.user_inputs['start_time_step'])
            & (regular_outputs.time_step - self.microsim.initializer.user_inputs['start_time_step']
               < affected_time_frame)
        ].groupby('time_step').sum().total_population
