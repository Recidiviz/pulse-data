...
# =============================================================================
"""Tests the sentence_group_projected_date_sessions view in sentence_sessions."""
...
class SentenceGroupProjectedDateSessionsTest(SimpleBigQueryViewBuilderTestCase):
    """Tests the SENTENCE_GROUP_PROJECTED_DATE_SESSIONS_VIEW_BUILDER"""
    ...
    sentence_group_length_address = queryable_address_for_normalized_entity(
        NormalizedStateSentenceGroupLength
    )
    ...
    def view_builder(self) -> SimpleBigQueryViewBuilder:
        return SENTENCE_GROUP_PROJECTED_DATE_SESSIONS_VIEW_BUILDER
    ...
    def parent_schemas(self) -> dict[BigQueryAddress, list[bigquery.SchemaField]]:
        return {
            ...
            self.sentence_group_length_address: get_bq_schema_for_entity_table(
                normalized_state_module,
                assert_subclass(
                    NormalizedStateSentenceGroupLength, Entity
                ).get_table_id(),
            ),
        }

    def test_group_serving_period_built_from_offset_sentences(self) -> None:
        """
        Checks with overlapping sentence serving periods for sentences in the same group, that the entire group serving
        period (union of the sentence serving periods) is captured in the output.
        """

        # The actual group length isn't relevant for this test
        sentence_group_length_data: List = []

        # sentence 1 and 2 are in the same sentence group. sentence 1 is served from critical date 1 to critical date 3
        # and sentence 2 starts being served on critical date 2 and is still open.
        ...
        # The output captures the period that any sentence in the group is served (critical date 1 to present)
        expected_data = [
            {
                "state_code": self.state_code.value,
                "person_id": self.person_id,
        ...
        self.run_simple_view_builder_query_test_from_data(
            {
                ...
                self.sentence_group_length_address: sentence_group_length_data,
                self.sentences_and_charges_address: sentences_and_charges_data,
            },
            expected_data,
        )

    def test_group_serving_period_construction_from_offset_sentences_different_groups(
        self,
    ) -> None:
        """
        If sentences are labeled as being in different groups, they should not be aggregated together, even if they
        overlap. This test confirms that.
        """
    ...
    def test_length_update_before_serving_start(self) -> None:
        ...
        sentence_group_length_data = [
            {
                "state_code": self.state_code.value,
                "person_id": self.person_id,
                "sentence_group_id": self.sentence_group_id_1,
                "group_update_datetime": self.early_datetime,
                "projected_full_term_release_date_max_external": self.projected_date_3_max,
            },
        ]
    ...
    def test_null_projected_date_update(self) -> None:
        """
        Test that when a sentence group gets an update that turns a non-null projected date to a null projected date,
        ...
        sentence_group_length_data = [
            ...
            },
            {
                ...
                "projected_full_term_release_date_max_external": None,
            },
        ]
