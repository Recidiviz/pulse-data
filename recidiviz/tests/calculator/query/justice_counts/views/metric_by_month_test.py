...
class PrisonPopulationViewTest(BaseViewTest):
    ...
    def test_multiple_filters(self) -> None:
        ...
        self.create_mock_bq_table(
            ...
            mock_data=pd.DataFrame(
                ...
                [[1, 'CORRECTIONS', 'POPULATION', 'INSTANT', ['global/location/state', 'metric/population/type'],
                  ['US_AA', 'SUPERVISION'], ['metric/supervision/type', 'metric/supervision/type/raw']],
                 # Aggregated by both filters
                 [2, 'CORRECTIONS', 'POPULATION', 'INSTANT', ['global/location/state'], ['US_XX'],
                  ['metric/population/type', 'metric/supervision/type', 'metric/supervision/type/raw']],
                 # Filtered by both filters
                 [3, 'CORRECTIONS', 'POPULATION', 'INSTANT',
                  ['global/location/state', 'metric/population/type', 'metric/supervision/type'],
                  ['US_YY', 'SUPERVISION', 'PAROLE'], ['global/gender']],
                 # Filtered incorrectly
                 [4, 'CORRECTIONS', 'POPULATION', 'INSTANT',
                  ['global/location/state', 'metric/population/type', 'metric/supervision/type'],
                  ['US_ZZ', 'SUPERVISION', 'PROBATION'], ['global/gender']]],
                columns=['id', 'system', 'metric_type', 'measurement_type', 'filtered_dimensions',
                         'filtered_dimension_values', 'aggregated_dimensions']))
        self.create_mock_bq_table(
            dataset_id='justice_counts', table_id='report_table_instance_materialized',
    ...
    def test_aggregated(self) -> None:
        ...
        self.create_mock_bq_table(
            ...
            mock_data=pd.DataFrame(
                # Matches
                [[1, 'CORRECTIONS', 'POPULATION', 'INSTANT', ['global/location/state', 'metric/population/type'],
                  ['US_XX', 'PRISON'], ['global/gender', 'global/gender/raw']],
                 # Not aggregated
                 [2, 'CORRECTIONS', 'POPULATION', 'INSTANT', ['global/location/state', 'metric/population/type'],
                  ['US_YY', 'PRISON'], []],
                 # Filtered instead, not comprehensive
                 [3, 'CORRECTIONS', 'POPULATION', 'INSTANT',
                  ['global/location/state', 'metric/population/type', 'global/gender'],
                  ['US_ZZ', 'PRISON', 'FEMALE'], []],
    ...
    def test_multiple_tables_same_source(self) -> None:
        ...
        self.create_mock_bq_table(
            ...
            mock_data=pd.DataFrame(
                [[1, 'CORRECTIONS', 'POPULATION', 'INSTANT', ['global/location/state', 'metric/population/type'],
                  ['US_XX', 'SUPERVISION'], ['metric/supervision/type']],
    ...
    def test_aggregate_with_nulls(self) -> None:
        ...
        self.create_mock_bq_table(
            ...
            mock_data=pd.DataFrame(
                [[1, 'CORRECTIONS', 'POPULATION', 'INSTANT', ['global/location/state', 'metric/population/type'],
                  ['US_XX', 'PRISON'], ['global/race', 'global/ethnicity']]],
                columns=['id', 'system', 'metric_type', 'measurement_type', 'filtered_dimensions',
                         'filtered_dimension_values', 'aggregated_dimensions']))
        self.create_mock_bq_table(
            dataset_id='justice_counts', table_id='report_table_instance_materialized',
        ...
        parole_population = metric_by_month.CalculatedMetricByMonth(
           ...
           aggregated_dimensions={
               'state_code': metric_by_month.Aggregation(dimension=manual_upload.State, comprehensive=False),
               'race': metric_by_month.Aggregation(dimension=manual_upload.Race, comprehensive=True)},
           output_name='POP'
        )
        results = self.query_view(
            metric_by_month.CalculatedMetricByMonthViewBuilder(
        ...
        self.create_mock_bq_table(
            ...
            mock_data=pd.DataFrame(
                ...
                # December - reports on Mondays, doesn't align with month, skipped
                 ...
                 [10, 2, 1, '2021-02-07', '2021-03-01', None]], # different source should not be summed
                columns=['id', 'report_id', 'report_table_definition_id', 'time_window_start', 'time_window_end',
                         'methodology']))
        self.create_mock_bq_table(
            dataset_id='justice_counts', table_id='cell_materialized',
    ...
    def test_average_less_than_month(self) -> None:
        ...
        self.create_mock_bq_table(
            ...
            mock_schema=MockTableSchema.from_sqlalchemy_table(schema.ReportTableDefinition.__table__),
            mock_data=pd.DataFrame(
                [[1, 'CORRECTIONS', 'POPULATION', 'AVERAGE', ['global/location/state'], ['US_XX'], []]],
                columns=['id', 'system', 'metric_type', 'measurement_type', 'filtered_dimensions',
                         'filtered_dimension_values', 'aggregated_dimensions']))
        self.create_mock_bq_table(
            dataset_id='justice_counts', table_id='report_table_instance_materialized',
    ...
    def test_comparisons(self) -> None:
        ...
        self.create_mock_bq_table(
            ...
            mock_schema=MockTableSchema.from_sqlalchemy_table(schema.ReportTableDefinition.__table__),
            mock_data=pd.DataFrame(
                [[1, 'CORRECTIONS', 'POPULATION', 'INSTANT', ['global/location/state'], ['US_XX'], []]],
                columns=['id', 'system', 'metric_type', 'measurement_type', 'filtered_dimensions',
                         'filtered_dimension_values', 'aggregated_dimensions']))
        self.create_mock_bq_table(
            dataset_id='justice_counts', table_id='report_table_instance_materialized',
            ...
            mock_data=pd.DataFrame(
                [[1, 1, 1, '2022-01-01', '2022-01-02', None], # Start of 2022-01
                 ...
                 [4, 1, 1, '2019-02-01', '2019-02-02', None], # Start of 2019-02 -- not quite a year back
                 [5, 1, 1, '2018-02-01', '2018-02-02', None], # Start of 2018-02 -- exactly a year prior
                 [6, 1, 1, '2017-03-01', '2017-03-02', None]],# Start of 2017-03 -- still not quite
                columns=['id', 'report_id', 'report_table_definition_id', 'time_window_start', 'time_window_end',
                         'methodology']))
        self.create_mock_bq_table(
            dataset_id='justice_counts', table_id='cell_materialized',
