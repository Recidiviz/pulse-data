...
class DataflowMonitorManagerTest(unittest.TestCase):
    ...
    def test_handle_dataflow_monitor_task_success(
        self, mock_get_job, mock_cloud_task_manager, mock_pubsub_helper, mock_project_id
    ):
        """Tests that a message is published to a Pub/Sub topic when a Dataflow
        job is found that has successfully completed."""
        mock_get_job.return_value = {"currentState": "JOB_STATE_DONE"}
        ...
        mock_cloud_task_manager.return_value.create_dataflow_monitor_task.assert_not_called()
        mock_pubsub_helper.publish_message_to_topic.assert_called_with(message, topic)

    @mock.patch("recidiviz.utils.metadata.project_id")
    @mock.patch(f"{MONITOR_MANAGER_PACKAGE_NAME}.pubsub_helper")
    @mock.patch(f"{MONITOR_MANAGER_PACKAGE_NAME}.CalculateCloudTaskManager")
    @mock.patch(f"{MONITOR_MANAGER_PACKAGE_NAME}.get_dataflow_job_with_id")
    def test_handle_dataflow_monitor_task_running(
        self, mock_get_job, mock_cloud_task_manager, mock_pubsub_helper, mock_project_id
    ):
        """Tests that a new Dataflow monitor task is created when the Dataflow
        job is still running."""
        mock_get_job.return_value = {"currentState": "JOB_STATE_RUNNING"}
        ...
        mock_cloud_task_manager.return_value.create_dataflow_monitor_task.assert_called_with(
    ...
    @mock.patch(f"{MONITOR_MANAGER_PACKAGE_NAME}.CalculateCloudTaskManager")
    @mock.patch(f"{MONITOR_MANAGER_PACKAGE_NAME}.get_dataflow_job_with_id")
    def test_handle_dataflow_monitor_task_stopped(
        self, mock_get_job, mock_cloud_task_manager, mock_pubsub_helper, mock_project_id
    ):
        """Tests that a new Dataflow monitor task is created when the Dataflow
        job is still temporarily stopped."""
        mock_get_job.return_value = {"currentState": "JOB_STATE_STOPPED"}
    ...
    @mock.patch(f"{MONITOR_MANAGER_PACKAGE_NAME}.CalculateCloudTaskManager")
    @mock.patch(f"{MONITOR_MANAGER_PACKAGE_NAME}.get_dataflow_job_with_id")
    def test_handle_dataflow_monitor_task_pending(
        self, mock_get_job, mock_cloud_task_manager, mock_pubsub_helper, mock_project_id
    ):
        """Tests that a new Dataflow monitor task is created when the Dataflow
        job is pending."""
        mock_get_job.return_value = {"currentState": "JOB_STATE_PENDING"}
    ...
    @mock.patch(f"{MONITOR_MANAGER_PACKAGE_NAME}.CalculateCloudTaskManager")
    @mock.patch(f"{MONITOR_MANAGER_PACKAGE_NAME}.get_dataflow_job_with_id")
    def test_handle_dataflow_monitor_task_queued(
        self, mock_get_job, mock_cloud_task_manager, mock_pubsub_helper, mock_project_id
    ):
        """Tests that a new Dataflow monitor task is created when the Dataflow
        job is queued."""
        mock_get_job.return_value = {"currentState": "JOB_STATE_QUEUED"}
    ...
    @mock.patch(f"{MONITOR_MANAGER_PACKAGE_NAME}.CalculateCloudTaskManager")
    @mock.patch(f"{MONITOR_MANAGER_PACKAGE_NAME}.get_dataflow_job_with_id")
    def test_handle_dataflow_monitor_task_failed(
        self, mock_get_job, mock_cloud_task_manager, mock_pubsub_helper, mock_project_id
    ):
        """Tests that no message is published and no new monitor tasks are
        created when the job has failed."""
        mock_get_job.return_value = {"currentState": "JOB_STATE_FAILED"}
