...
def manifest_filepath(report_id: str, manifest_name: Optional[str] = None) -> str:
    return os.path.join(
        ...
        "fixtures",
        report_id,
        manifest_name or "manifest.yaml",
    )
...
class ManualUploadTest(TestCase):
    ...
    def test_ingestReport_isPersisted(self) -> None:
        ...
        manual_upload.ingest(
            self.fs, test_utils.prepare_files(self.fs, manifest_filepath("report1"))
        )
    ...
    def test_ingestAndUpdateReport_isPersisted(self) -> None:
        ...
        manual_upload.ingest(
            self.fs,
            test_utils.prepare_files(self.fs, manifest_filepath("report1_updated")),
        )
    ...
    def test_ingestMultiDimensionReport_isPersisted(self) -> None:
        ...
        manual_upload.ingest(
            ...
            test_utils.prepare_files(
                self.fs, manifest_filepath("report2_multidimension")
            ),
        )
    ...
    def _test_ingestReport_dynamicSnapshot(self, report_id: str) -> None:
        ...
        # Act
        manual_upload.ingest(
            self.fs, test_utils.prepare_files(self.fs, manifest_filepath(report_id))
        )
    ...
    def test_ingestSubtypeNotStrict_isPersisted(self) -> None:
        ...
        manual_upload.ingest(
            self.fs,
            test_utils.prepare_files(self.fs, manifest_filepath("report5_subtype")),
        )
    ...
    def test_ingestSubtypeStrict_isNotPersisted(self) -> None:
        ...
        with self.assertRaises(EnumParsingError):
            manual_upload.ingest(
                self.fs,
                test_utils.prepare_files(
                    self.fs, manifest_filepath("report5_subtype_fail")
                ),
            )
    ...
    def test_ingestAdditionalFilters_isPersisted(self) -> None:
        ...
        manual_upload.ingest(
            self.fs,
            test_utils.prepare_files(self.fs, manifest_filepath("report6_filters")),
        )
    ...
    def test_supportCommaNumbers_isPersisted(self) -> None:
        ...
        manual_upload.ingest(
            self.fs,
            test_utils.prepare_files(self.fs, manifest_filepath("report6_commas")),
        )
    ...
    def test_ingestReport_populationTypeDimension(self) -> None:
        ...
        manual_upload.ingest(
            self.fs,
            test_utils.prepare_files(
                self.fs, manifest_filepath("report7_population_types")
            ),
        )
    ...
    def test_ingestReport_parolePopulation(self) -> None:
        ...
        manual_upload.ingest(
            self.fs,
            test_utils.prepare_files(
                self.fs, manifest_filepath("report_parole_population")
            ),
        )
    ...
    def test_raiseError_noPopulationTypeDimensionOrMetric(self) -> None:
        with self.assertRaisesRegex(
            ...
            manual_upload.ingest(
                self.fs,
                test_utils.prepare_files(
                    self.fs, manifest_filepath("report8_no_population_types_fail")
                ),
            )
    ...
    def test_admissionMetric_isPersisted(self) -> None:
        ...
        manual_upload.ingest(
            self.fs,
            test_utils.prepare_files(self.fs, manifest_filepath("report_admissions")),
        )
    ...
    def test_releasesMetric_isPersisted(self) -> None:
        ...
        manual_upload.ingest(
            self.fs,
            test_utils.prepare_files(self.fs, manifest_filepath("report_releases")),
        )
    ...
    def test_reincarcerationsWithViolationType_arePersisted(self) -> None:
        ...
        manual_upload.ingest(
            self.fs,
            test_utils.prepare_files(
                self.fs, manifest_filepath("report8_reincarcerations")
            ),
        )
    ...
    def test_ingestReport_fixed_range_month(self) -> None:
        ...
        manual_upload.ingest(
            self.fs,
            test_utils.prepare_files(
                self.fs, manifest_filepath("report6_fixed_month_range")
            ),
        )
    ...
    def test_ingestReport_fixed_range_year(self) -> None:
        ...
        manual_upload.ingest(
            self.fs,
            test_utils.prepare_files(
                self.fs, manifest_filepath("report6_fixed_year_range")
            ),
        )
    ...
    def test_ingestReport_synthetic_column(self) -> None:
        ...
        manual_upload.ingest(
            self.fs,
            test_utils.prepare_files(
                self.fs, manifest_filepath("report_synthetic_column")
            ),
        )
    ...
    def test_ingestAgeRaw_isPersisted(self) -> None:
        ...
        manual_upload.ingest(
            self.fs,
            test_utils.prepare_files(self.fs, manifest_filepath("report_age_raw")),
        )
    ...
    def test_DynamicDateRangeProducer_convert_chronolicalError(self) -> None:
        ...
        columns = {
            "Start": manual_upload.DATE_FORMAT_PARSERS[
                manual_upload.DateFormatType.MONTH
            ],
            "End": manual_upload.DATE_FORMAT_PARSERS[
                manual_upload.DateFormatType.MONTH
            ],
        }
        args = ("2019-01", "2018-01")
        dynamic_range_producer = manual_upload.DynamicDateRangeProducer(
    ...
    def test_jailPopulationAndFIPS_arePersisted(self) -> None:
        ...
        manual_upload.ingest(
            self.fs,
            test_utils.prepare_files(
                self.fs, manifest_filepath("report_jail_population")
            ),
        )
    ...
    def test_jailPopulationAndFIPSMultipleStates_arePersisted(self) -> None:
        ...
        manual_upload.ingest(
            self.fs,
            test_utils.prepare_files(
                self.fs, manifest_filepath("report_jail_population_multiple_states")
            ),
        )
    ...
    def test_NaNs_fail(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "Invalid value 'NaN'"):
            manual_upload.ingest(
                self.fs,
                test_utils.prepare_files(self.fs, manifest_filepath("report_nans")),
            )
    ...
    def test_reingestReport(self) -> None:
        ...
        manual_upload.ingest(
            self.fs,
            test_utils.prepare_files(
                self.fs, manifest_filepath("report_reingest/original")
            ),
        )
