...
class TestBasicSingleTaskEligibilitySpansBigQueryViewCollector(unittest.TestCase):
    """Tests for the BasicSingleTaskEligibilitySpansBigQueryViewCollector."""
    ...
    def test_collect_all(self) -> None:
        collector = BasicSingleTaskEligibilitySpansBigQueryViewCollector(
            self.tes_builders
        )
        all_basic_eligibility_builders = collector.collect_view_builders()

        # Fail if we didn't find any builders
        self.assertGreater(len(all_basic_eligibility_builders), 0)

        for builder in all_basic_eligibility_builders:
            if not isinstance(
                builder, BasicSingleTaskEligibilitySpansBigQueryViewBuilder
            ):
                raise ValueError(
                    f"Found unexpected basic task eligibility view builder type "
        ...
        for state_code, builders in all_task_eligibility_builders_by_state.items():
            ...
            for builder in builders:
                if not isinstance(
                    ...
                    raise ValueError(
                        f"Found unexpected basic eligibility view builder type "
                        f"[{type(builder)}]: {builder}"
                    )
