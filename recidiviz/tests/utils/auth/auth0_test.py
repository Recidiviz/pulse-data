...
def get_public_jwk(public_key, key_id):
    key = json.loads(RSAAlgorithm.to_jwk(RSAAlgorithm(RSAAlgorithm.SHA256).prepare_key(public_key)))
    key['alg'] = 'RS256'
    key['kid'] = key_id
    return key
...
class Auth0ModuleTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.urlopen_patcher.start().return_value = Mock(read=lambda: json.dumps({"keys": [self.public_jwk]}))
    ...
    def test_jwt_authorization_header(self) -> None:
        ...
        self.assertEqual(response.get_json()["description"], "Authorization header is expected")
        ...
        self.assertEqual(response.get_json()["description"], "Authorization header must start with Bearer")
        ...
        self.assertEqual(response.status_code, 401)
        self.assertEqual(response.get_json()["code"], "invalid_header")
        self.assertEqual(response.get_json()["description"], "Authorization header must be Bearer token")
    ...
    def test_jwt_contents(self) -> None:
        ...
        self.assertEqual(response.get_json()["description"], "incorrect claims, please check the audience and issuer")
        self.assertEqual(response.status_code, 401)
        ...
        token = self.build_jwt({'iss': 'none', 'aud': self.authorization_config.audience})
        response = self.subject({"Authorization": f"Bearer {token}"})
        self.assertEqual(response.get_json()["code"], "invalid_claims")
        self.assertEqual(response.get_json()["description"], "incorrect claims, please check the audience and issuer")
        self.assertEqual(response.status_code, 401)
        ...
        token = self.build_jwt({'iss': self.authorization_config.issuer, 'aud': self.authorization_config.audience})
        response = self.subject({"Authorization": f"Bearer {token}"})
