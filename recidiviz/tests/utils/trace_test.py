...
class TestSpan(unittest.TestCase):
    ...
    def test_call_recordsTime(self, mock_time, mock_measurements_method):
        ...
        mock_measurements_method.assert_called_with(
            {'module': 'recidiviz.tests.utils.trace_test', 'function': 'some_method', 'recursion_depth': 0})
        self.assertEqual([call(ANY, pytest.approx(0.15))], mock_measurements.measure_float_put.mock_calls)
    ...
    def test_recursive_countsDepth(self, mock_measurements_method):
        ...
        mock_measurements_method.assert_has_calls([
            call({'module': 'recidiviz.tests.utils.trace_test',
                  'function': 'SomeClass.recursive', 'recursion_depth': 0}),
            call().__enter__(),
            call({'module': 'recidiviz.tests.utils.trace_test',
                  'function': 'SomeClass.recursive', 'recursion_depth': 1}),
            call().__enter__(),
            call({'module': 'recidiviz.tests.utils.trace_test',
                  'function': 'SomeClass.recursive', 'recursion_depth': 2}),
            call().__enter__(),
            call({'module': 'recidiviz.tests.utils.trace_test',
                  'function': 'SomeClass.recursive', 'recursion_depth': 3}),
            call().__enter__(),
        ])
...
class TestCompositeSampler(unittest.TestCase):
    ...
    def test_compositeSampler_picksCorrect(self, _name, test_input, expected, mock_request):
        # Arrange
        mock_1 = Mock()
