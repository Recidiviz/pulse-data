...
class TestRunValidations(StateIngestPipelineTestCase):
    ...
    def test_validate_entity_tree_unique_constraint_exemption_present(self) -> None:
        person = StatePerson(
            state_code="US_XX",
            person_id=3111,
            external_ids=[
    ...
    @patch(
        "recidiviz.pipelines.ingest.state.run_validations.GLOBAL_UNIQUE_CONSTRAINT_ERROR_EXEMPTIONS",
        {StateCode.US_XX: {"supervision_contact_external_ids_unique_within_state"}},
    )
    def test_validate_global_unique_constraint_exemption_present(self) -> None:
        person1 = StatePerson(
            state_code="US_XX",
            person_id=1237,
        )
