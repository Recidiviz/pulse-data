...
class TestJusticePublisherAdminPanelAPI(JusticeCountsDatabaseTestCase):
    ...
    def test_update_custom_child_agency_name(self) -> None:
        ...
        response = self.client.put(
            f"/admin/agency/{self.agency_A_id}/custom-name",
        ...
        self.assertEqual(self.agency_A_id, response_json["id"])

        agency_A = AgencyInterface.get_agency_by_id(
            session=self.session, agency_id=self.agency_A_id
        )
        ...
        child_agency_1_id = child_agency_1.id
        response = self.client.put(
            f"/admin/agency/{child_agency_1_id}/custom-name",
            json={"custom_child_agency_name": "Agency Incredible"},
        )
        self.assertEqual(response.status_code, 200)
        response_json = assert_type(response.json, dict)
