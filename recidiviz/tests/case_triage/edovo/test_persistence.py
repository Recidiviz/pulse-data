...
# External, DOC-facing id as Edovo sends it (the CO ADC number).
_DOC_ID = "A123456"

_RECEIVED_AT = datetime(2026, 6, 8, 12, 0, 0, tzinfo=timezone.utc)
...
class TestPersistCompletion(TestCase):
    ...
    def test_new_request_is_written_and_returns_is_new(self) -> None:
        ...
        with SessionFactory.using_database(self.database_key) as session:
            record, is_new = persist_completion(
                session, request, idempotency_key, _RECEIVED_AT
            )
            self.assertTrue(is_new)
            self.assertIsNotNone(record.id)
            self.assertEqual(record.person_external_id, _DOC_ID)
            self.assertEqual(record.id_type, "US_CO_ADCNUMBER")
    ...
    def test_received_at_from_route_is_persisted(self) -> None:
        request = CourseCompletionRequest.model_validate(_VALID_PAYLOAD)
        idempotency_key = uuid.uuid4()

        with SessionFactory.using_database(self.database_key) as session:
            persist_completion(session, request, idempotency_key, _RECEIVED_AT)
            session.commit()
            persisted = (
            ...
            # The route-supplied received_at is stored, not just a DB-default
            # insert timestamp.
            self.assertEqual(persisted.received_at, _RECEIVED_AT)

    def test_no_redundant_created_at_column(self) -> None:
        # created_at was dropped as redundant with received_at; guard against
        # it creeping back.
        self.assertNotIn("created_at", EdovoCourseCompletion.__table__.columns)

    def test_content_hours_stored_as_float(self) -> None:
        request = CourseCompletionRequest.model_validate(
            {**_VALID_PAYLOAD, "content_hours": 4.5}
        )
        idempotency_key = uuid.uuid4()
        ...
        with SessionFactory.using_database(self.database_key) as session:
            ...
            self.assertIsInstance(persisted.content_hours, float)
            self.assertEqual(persisted.content_hours, 4.5)
    ...
    def test_different_courses_for_same_person_are_independent(self) -> None:
        ...
        with SessionFactory.using_database(self.database_key) as session:
            record_a, is_new_a = persist_completion(
                session, request_a, uuid.uuid4(), _RECEIVED_AT
            )
        ...
        with SessionFactory.using_database(self.database_key) as session:
            record_b, is_new_b = persist_completion(
                session, request_b, uuid.uuid4(), _RECEIVED_AT
            )
