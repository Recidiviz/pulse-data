...
class TestScraperStart(unittest.TestCase):
    ...
    def test_start(
        ...
        response = self.client.get("/start", query_string=request_args, headers=headers)
        ...
        mock_update_phase.assert_called_with(
            mock_create_session.return_value, scrape_phase.ScrapePhase.SCRAPE
        )
...
class TestScraperResume(unittest.TestCase):
    ...
    def test_resume(self, mock_sessions, mock_region, mock_supported):
        ...
        response = self.client.get(
            "/resume", query_string=request_args, headers=headers
        )
        assert response.status_code == 200
