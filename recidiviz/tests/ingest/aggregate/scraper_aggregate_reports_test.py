...
app = Flask(__name__)
app.register_blueprint(scrape_aggregate_reports.scrape_aggregate_reports_blueprint)
...
class TestScraperAggregateReports(TestCase):
    ...
    def setup_method(self, _test_method):
        ...
        self.upload_bucket = scrape_aggregate_reports.UPLOAD_BUCKET.format(TEST_ENV)
    ...
    def testExistsNoUpload(self, mock_get_all_tx, mock_open, mock_get, mock_fs):
        ...
        mock_fs.assert_called_with(project=TEST_ENV, cache_timeout=GCSFS_NO_CACHING)
    ...
    def testCaNoExistsUpload200(self, mock_get_all_ca, mock_open, mock_post, mock_fs):
        ...
        mock_fs.assert_called_with(project=TEST_ENV, cache_timeout=GCSFS_NO_CACHING)
        mock_fs_return.exists.assert_called_with(
            os.path.join(self.historical_bucket, "california", EXISTING_CA_NAME)
        )
        mock_fs_return.put.assert_called_with(temploc, upload_bucket)
        mock_open.assert_called_with(temploc, "wb")
    ...
    def testMultipleUrlsAll200(self, mock_get_all_tx, mock_open, mock_get, mock_fs):
        ...
        expected_exists_calls = [
            call(os.path.join(self.historical_bucket, "texas", EXISTING_PDF_NAME)),
            call(os.path.join(self.historical_bucket, "texas", EXISTING_PDF_NAME2)),
        ]
        self.assertCountEqual(
            mock_fs_return.exists.call_args_list, expected_exists_calls
        )
        expected_put_calls = [
            call(temploc1, upload_bucket1),
            call(temploc2, upload_bucket2),
        ]
        self.assertCountEqual(mock_fs_return.put.call_args_list, expected_put_calls)
        expected_open_calls = [call(temploc1, "wb"), call(temploc2, "wb")]
    ...
    def testMultipleUrlsOne200OneNoExists(
        ...
        mock_fs.assert_called_with(project=TEST_ENV, cache_timeout=GCSFS_NO_CACHING)
        self.assertEqual(mock_fs.call_count, 1)
        expected_exists_calls = [call(historical_path1), call(historical_path2)]
        self.assertCountEqual(
            mock_fs_return.exists.call_args_list, expected_exists_calls
        )
