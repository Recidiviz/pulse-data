...
class TestDirectIngestGcsFileSystem(TestCase):
    ...
    def fully_process_file(
        ...
        if file_type_differentiation_on:
            ...
            self.assertEqual(
                len(end_raw_storage_paths), len(start_raw_storage_paths) + 1
            )
            self.assertEqual(
...
class TestPathNormalization(TestCase):
    ...
    def test_to_normalized_processed_file_path_from_normalized_path(self) -> None:
        ...
        self.assertEqual(
            expected_file_path,
            to_normalized_processed_file_path_from_normalized_path(
                original_file_path, GcsfsDirectIngestFileType.RAW_DATA
            ),
        )
