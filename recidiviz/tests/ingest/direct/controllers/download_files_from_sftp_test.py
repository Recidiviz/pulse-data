...
from recidiviz.ingest.direct.controllers.download_files_from_sftp import SftpAuth, \
    DownloadFilesFromSftpController, RAW_INGEST_DIRECTORY
from recidiviz.ingest.direct.sftp_download_delegate_factory import SftpDownloadDelegateFactory
from recidiviz.tests.cloud_storage.fake_gcs_file_system import FakeGCSFileSystem

TODAY = datetime.datetime.today()
YESTERDAY = datetime.datetime.today() - datetime.timedelta(1)
TWO_DAYS_AGO = datetime.datetime.today() - datetime.timedelta(2)
TEST_SSH_RSA_KEY = "AAAAB3NzaC1yc2EAAAADAQABAAABAQCaqqwHqIxyLJwk5ppScpxjGIr9YeGNtWL/Ci0cYKMtUBWrIcosPMnNkyR/" \
...
def create_files(download_dir: str, remotedir: str, gcsfs: FakeGCSFileSystem) -> FakeGCSFileSystem:
    gcsfs.test_add_path(path=GcsfsDirectoryPath.from_absolute_path(f'{download_dir}/{remotedir}'), local_path=None)
    for item in INNER_FILE_TREE:
        path_to_add = f'{download_dir}/{remotedir}/{item}'
        if os.path.isdir(item):
            gcsfs.test_add_path(path=GcsfsDirectoryPath.from_absolute_path(path_to_add), local_path=None)
        else:
            gcsfs.test_add_path(path=GcsfsFilePath.from_absolute_path(path_to_add), local_path=None)
    return gcsfs
...
def mock_init_connection_options(self: CnOpts) -> None:
    ...
    self.hostkeys.add("testhost.ftp", "ssh-rsa", RSAKey(data=decodebytes(bytes(TEST_SSH_RSA_KEY, 'utf-8'))))


def mock_walktree(remotepath: str,
...
class TestSftpAuth(unittest.TestCase):
    ...
    def test_initialization(self, mock_secret: MagicMock) -> None:
        ...
        self.assertTrue(result.connection_options.hostkeys.lookup("testhost.ftp") is not None)
    ...
    @patch("recidiviz.utils.secrets.get_secret")
    def test_initialization_connection_options_error(self, mock_secret: MagicMock) -> None:
        test_secrets = {
            "us_xx_sftp_host": "testhost.ftp",
            "us_xx_sftp_username": "username",
            "us_xx_sftp_password": "password"
    ...
    @patch("recidiviz.utils.secrets.get_secret")
    def test_set_up_connection_options_empty_hostkey_error(self, mock_secret: MagicMock) -> None:
        test_secrets = {
            "us_xx_sftp_host": "testhost.ftp",
    ...
    def test_set_up_connection_options_uses_known_hosts_first(self, mock_secret: MagicMock) -> None:
        test_secrets = {
            "us_xx_sftp_hostkey": "testhost.ftp ssh-rsa " + TEST_SSH_RSA_KEY
        }
        mock_secret.side_effect = test_secrets.get
...
@patch.object(SftpDownloadDelegateFactory, "build", return_value=TestSftpDownloadDelegate())
@patch.object(SftpAuth, "for_region", return_value=SftpAuth("testhost.ftp", "username", "password", CnOpts()))
...
class TestDownloadFilesFromSftpController(unittest.TestCase):
    ...
    def test_get_paths_to_download(self,
                                   _mock_connection: Mock,
                                   mock_fs_factory: Mock,
                                   _mock_auth: Mock,
                                   _mock_download: Mock) -> None:
    ...
    def test_do_fetch_succeeds(self,
                               _mock_connection: Mock,
                               mock_fs_factory: Mock,
                               _mock_auth: Mock,
                               _mock_download: Mock) -> None:
        ...
        self.assertListEqual(items, [(
            os.path.join("test-project-direct-ingest-state-us-xx", RAW_INGEST_DIRECTORY, "testToday", item),
            TODAY) for item in INNER_FILE_TREE if ".txt" in item])
    ...
    def test_do_fetch_gets_all_files_if_no_lower_bound_date(self,
                                                            _mock_connection: Mock,
                                                            mock_fs_factory: Mock,
                                                            _mock_auth: Mock,
                                                            _mock_download: Mock
        ...
        items, _ = controller.do_fetch()
        self.assertSetEqual(set(items), set(itertools.chain(*[
            [
                (os.path.join("test-project-direct-ingest-state-us-xx", RAW_INGEST_DIRECTORY, "testToday", item),
                 TODAY),
                (os.path.join("test-project-direct-ingest-state-us-xx", RAW_INGEST_DIRECTORY, "testTwoDaysAgo", item),
                 TWO_DAYS_AGO)
            ] for item in INNER_FILE_TREE if ".txt" in item])))
        self.assertEqual(len(mock_fs.files), len(items))
    ...
    def test_do_fetch_graceful_file_failures(self, _mock_connection: Mock,
                                             mock_fs_factory: Mock,
                                             _mock_auth: Mock,
                                             _mock_download: Mock) -> None:
        ...
        self.assertEqual(items,
                         [(os.path.join("test-project-direct-ingest-state-us-xx",
                                        RAW_INGEST_DIRECTORY, "testToday", "file1.txt"), TODAY)]
                         )
        self.assertEqual(len(mock_fs.files), len(items))

    def test_clean_up_succeeds(self,
                               mock_fs_factory: Mock,
                               _mock_auth: Mock,
                               _mock_download: Mock) -> None:
        mock_fs = create_files(download_dir=f"test-project-direct-ingest-state-us-xx/{RAW_INGEST_DIRECTORY}",
                               remotedir="testToday", gcsfs=FakeGCSFileSystem())
        mock_fs_factory.return_value = mock_fs
    ...
    def test_clean_up_fails_gracefully(self,
                                       mock_fs_factory: Mock,
                                       _mock_auth: Mock,
                                       _mock_download: Mock) -> None:
        mock_fs = BrokenGCSFSFakeSystem()
