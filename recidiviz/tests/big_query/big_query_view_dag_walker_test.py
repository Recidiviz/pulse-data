...
class TestBigQueryViewDagWalker(unittest.TestCase):
                ...
                if not node.is_root:
                    for parent_key in node.parent_keys:
                        if parent_key not in all_processed:
                            ...
                            try:
                                self.assertIsValidSourceDataTable(child_view_key=node_key, source_table_key=parent_key)
    ...
    def test_dag_with_cycle_at_root(self) -> None:
        ...
        self.assertEqual(str(e.exception), "No roots detected. Input views contain a cycle.")
    ...
    def assertIsValidRawTableViewDependency(
        ...
        region_raw_file_config = DirectIngestRegionRawFileConfig(region_code=region_code)
        raw_data_latest_view_match = re.match(r'([A-Za-z_\d]+)_latest', raw_data_view_table_id)
        if not raw_data_latest_view_match:
            raise ValueError(f'Unexpected table_id [{raw_data_view_table_id}] for parent view [{raw_data_view_key}] '
        ...
        if raw_file_tag not in region_raw_file_config.raw_file_configs:
            raise ValueError(f'No raw file config exists for raw data view [{raw_data_view_key}] referenced by view '
                             f'[{child_view_key}].')

        config = region_raw_file_config.raw_file_configs[raw_file_tag]
        if not config.primary_key_cols:
            raise ValueError(f'Raw file [{raw_file_tag}] has no primary key cols defined - the _latest view for '
...
class TestBigQueryViewDagNode(unittest.TestCase):
    ...
    def test_parse_view_multiple_parents(self) -> None:
        ...
        node = BigQueryViewDagNode(view)
        self.assertEqual(node.parent_keys, {('some_dataset', 'some_table'), ('some_dataset', 'other_table')})
