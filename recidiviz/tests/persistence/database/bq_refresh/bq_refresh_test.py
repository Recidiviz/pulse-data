...
class BqLoadTest(unittest.TestCase):
    ...
    @mock.patch(f"{BQ_REFRESH_PACKAGE_NAME}.delete_temp_table_if_exists")
    @mock.patch(
        f"{BQ_REFRESH_PACKAGE_NAME}.load_rows_excluded_from_refresh_into_temp_table_and_wait"
    )
    @mock.patch(f"{BQ_REFRESH_PACKAGE_NAME}.load_table_from_gcs_and_wait")
    def test_refresh_bq_table_from_gcs_export_synchronous(
        ...
        expected_calls = [
            mock.call.load_from_gcs(
                self.mock_bq_client,
                self.mock_table_id,
                self.mock_bq_refresh_config,
                destination_table_id=temp_table_name,
            ),
            mock.call.load_stale_data(
                self.mock_bq_client,
                self.mock_table_id,
                self.mock_bq_refresh_config,
                destination_table_id=temp_table_name,
            ),
            mock.call.delete_temp_table(
                self.mock_bq_client, temp_table_name, self.mock_bq_refresh_config
            ),
        ]

        bq_refresh.refresh_bq_table_from_gcs_export_synchronous(
            self.mock_bq_client, self.mock_table_id, self.mock_bq_refresh_config
        )
    ...
    def test_refresh_from_or_create_table_table_doesnt_exist(
        self, mock_load_stale_data: mock.MagicMock
    ) -> None:
        ...
        with self.assertLogs(level="INFO"):
            ...
            self.mock_bq_refresh_config.get_bq_schema_for_table.return_value = (
                self.mock_table_schema
            )
    ...
    @mock.patch(f"{BQ_REFRESH_PACKAGE_NAME}.delete_temp_table_if_exists")
    @mock.patch(f"{BQ_REFRESH_PACKAGE_NAME}.wait_for_table_load")
    def test_refresh_bq_table_from_gcs_export_synchronous_load_table_async_fails(
        self, mock_wait: mock.MagicMock, mock_delete: mock.MagicMock
    ) -> None:
    ...
    def test_load_rows_excluded_from_refresh_into_temp_table_and_wait(self) -> None:
        ...
        bq_refresh.load_rows_excluded_from_refresh_into_temp_table_and_wait(
            self.mock_bq_client,
            self.mock_table_id,
            self.mock_bq_refresh_config,
    ...
    def test_load_table_from_gcs_and_wait(self) -> None:
        ...
        self.mock_bq_refresh_config.get_dataset_ref.return_value = self.mock_dataset
        self.mock_bq_refresh_config.get_gcs_export_uri_for_table.return_value = (
        ...
        bq_refresh.load_table_from_gcs_and_wait(
            self.mock_bq_client,
            self.mock_table_id,
            self.mock_bq_refresh_config,
            destination_table_id,
    ...
    def test_delete_temp_table_if_exists(self) -> None:
        ...
        with self.assertLogs(level="INFO"):
            temp_table_name = f"{self.mock_table_id}_temp"
            self.mock_bq_refresh_config.dataset_id = self.mock_dataset_id
            bq_refresh.delete_temp_table_if_exists(
                self.mock_bq_client, temp_table_name, self.mock_bq_refresh_config
            )
            self.mock_bq_client.delete_table.assert_called_with(
                dataset_id=self.mock_dataset_id, table_id=temp_table_name
            )
