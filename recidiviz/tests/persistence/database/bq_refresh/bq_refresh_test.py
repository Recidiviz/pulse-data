...
class BqLoadTest(unittest.TestCase):
    ...
    @mock.patch(f'{BQ_REFRESH_PACKAGE_NAME}.load_rows_excluded_from_refresh_into_temp_table_and_wait')
    ...
    def test_refresh_bq_table_from_gcs_export_synchronous(self,
        ...
        bq_refresh.refresh_bq_table_from_gcs_export_synchronous(self.mock_bq_client, self.mock_table_id,
                                                                self.mock_bq_refresh_config)
    ...
    def test_refresh_bq_table_from_gcs_export_synchronous_load_from_gcs_fails(
        ...
        with self.assertRaises(ValueError):
            bq_refresh.refresh_bq_table_from_gcs_export_synchronous(self.mock_bq_client,
                                                                    self.mock_table_id,
                                                                    self.mock_bq_refresh_config)
    ...
    def test_load_rows_excluded_from_refresh_into_temp_table_and_wait(self) -> None:
        ...
        bq_refresh.load_rows_excluded_from_refresh_into_temp_table_and_wait(self.mock_bq_client, self.mock_table_id,
                                                                            self.mock_bq_refresh_config,
    ...
    def test_load_table_from_gcs_and_wait(self) -> None:
        ...
        bq_refresh.load_table_from_gcs_and_wait(self.mock_bq_client, self.mock_table_id, self.mock_bq_refresh_config,
                                                destination_table_id)
    ...
    def test_wait_for_table_load_fail(self) -> None:
        ...
        with self.assertLogs(level='ERROR'):
            success = bq_refresh.wait_for_table_load(self.mock_bq_client, self.mock_load_job)
    ...
    def test_delete_temp_table_if_exists(self) -> None:
        ...
        with self.assertLogs(level='INFO'):
            temp_table_name = f'{self.mock_table_id}_temp'
            self.mock_bq_refresh_config.dataset_id = self.mock_dataset_id
            bq_refresh.delete_temp_table_if_exists(self.mock_bq_client, temp_table_name,
                                                   self.mock_bq_refresh_config)
