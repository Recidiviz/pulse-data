...
class CloudSqlToGcsExportTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.metadata_patcher = mock.patch(
            f"{CLOUD_SQL_TO_GCS_EXPORT_PACKAGE_NAME}.metadata"
        )
        ...
        self.mock_get_operation = (
            self.mock_client.operations.return_value.get.return_value
        )
        ...
        self.mock_bq_refresh_config.get_table_export_query.return_value = (
    ...
    def test_wait_until_operation_finishes_api_fails_with_retries(
    ...
    @mock.patch(f"{CLOUD_SQL_TO_GCS_EXPORT_PACKAGE_NAME}.time.sleep")
    def test_wait_until_operation_finishes_api_fails_raises(
        self, mock_time: mock.MagicMock
    ) -> None:
        """Test that wait_until_operation_finished retries only 3 times until it raises an error."""
        mock_time.side_effect = None
        self.mock_get_operation.execute.side_effect = googleapiclient.errors.HttpError(
    ...
    @mock.patch(f"{CLOUD_SQL_TO_GCS_EXPORT_PACKAGE_NAME}.time.sleep")
    def test_wait_until_operation_finished_waits(
        self, mock_time: mock.MagicMock
    ) -> None:
    ...
    @mock.patch(f"{CLOUD_SQL_TO_GCS_EXPORT_PACKAGE_NAME}.time.sleep")
    def test_wait_until_operation_finished_surfaces_errors(
        self, mock_time: mock.MagicMock
    ) -> None:
    ...
    def test_export_table(self, mock_wait: mock.MagicMock) -> None:
        ...
        """
        cloud_sql_to_gcs_export.export_table(
        ...
        mock_export.assert_called_with(
            project=self.mock_project_id, instance=self.mock_instance_id, body=mock.ANY
        )
