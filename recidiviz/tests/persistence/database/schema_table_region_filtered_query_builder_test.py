...
from recidiviz.persistence.database.schema_table_region_filtered_query_builder import \
    CloudSqlSchemaTableRegionFilteredQueryBuilder, SchemaTableRegionFilteredQueryBuilder, \
...
class SchemaTableRegionFilteredQueryBuilderTest(unittest.TestCase):
    ...
    def test_format_region_codes_for_sql(self) -> None:
        ...
        """
        self.assertEqual(SchemaTableRegionFilteredQueryBuilder.format_region_codes_for_sql(self.single_state_code),
                         "'US_VA'")
        self.assertEqual(SchemaTableRegionFilteredQueryBuilder.format_region_codes_for_sql(self.multiple_states),
                         "'US_ND','US_ID'")
    ...
    def test_format_columns_for_sql(self) -> None:
        ...
        """
        self.assertEqual(
            SchemaTableRegionFilteredQueryBuilder.format_columns_for_sql(['name', 'state_code', 'person_id']),
            "name,state_code,person_id")
        self.assertEqual(SchemaTableRegionFilteredQueryBuilder.format_columns_for_sql([]), "")
    ...
    def test_format_columns_for_sql_with_prefix(self) -> None:
        ...
        self.assertEqual(
            SchemaTableRegionFilteredQueryBuilder.format_columns_for_sql(['name', 'state_code', 'person_id'],
                                                                         table_prefix),
            expected)
        self.assertEqual(SchemaTableRegionFilteredQueryBuilder.format_columns_for_sql([], table_prefix), "")
...
class CloudSqlSchemaTableRegionFilteredQueryBuilderTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        ForeignKeyConstraint = collections.namedtuple('ForeignKeyConstraint', ['referred_table', 'column_keys'])
        ...
        self.get_table_class_by_name_patcher.start().return_value = self.fake_state_table
    ...
    def test___init__(self) -> None:
        ...
        with self.assertRaises(ValueError):
            CloudSqlSchemaTableRegionFilteredQueryBuilder(
                StateBase, self.fake_jails_table, self.mock_columns_to_include,
                region_codes_to_include=[], region_codes_to_exclude=['US_ID'])
    ...
    def test_select_clause_state_schema(self) -> None:
        ...
        query_builder = CloudSqlSchemaTableRegionFilteredQueryBuilder(StateBase, self.fake_state_table,
                                                                      self.mock_columns_to_include)
    ...
    def test_select_clause_state_association_table(self) -> None:
        ...
        query_builder = CloudSqlSchemaTableRegionFilteredQueryBuilder(StateBase, self.fake_association_table,
                                                                      self.mock_association_table_columns_to_include)
        expected_select = f'SELECT {self.fake_association_table.name}.column1,' \
    ...
    def test_join_clause(self) -> None:
        ...
        query_builder = CloudSqlSchemaTableRegionFilteredQueryBuilder(StateBase, self.fake_association_table,
                                                                      self.mock_association_table_columns_to_include)
        expected_join = f'JOIN {self.fake_state_table.name} ON ' \
    ...
    def test_filter_clause_region_codes_to_exclude(self) -> None:
        """Given a StateBase schema and excluded region codes, it returns a filter clause to exclude the region codes.
        """
        query_builder = CloudSqlSchemaTableRegionFilteredQueryBuilder(StateBase, self.fake_state_table,
                                                                      self.mock_columns_to_include,
    ...
    def test_full_query(self) -> None:
        ...
        query_builder = CloudSqlSchemaTableRegionFilteredQueryBuilder(JailsBase, self.fake_jails_table,
                                                                      self.mock_columns_to_include)
        expected_query = f"SELECT {self.fake_jails_table.name}.column1,{self.fake_jails_table.name}.state_code " \
    ...
    def test_full_query_region_codes_to_exclude(self) -> None:
        ...
        expected_query = f"SELECT {self.fake_state_table.name}.column1,{self.fake_state_table.name}.state_code " \
    ...
    def test_full_query_association_table(self) -> None:
        ...
        query_builder = CloudSqlSchemaTableRegionFilteredQueryBuilder(StateBase, self.fake_association_table,
                                                                      self.mock_association_table_columns_to_include,
                                                                      region_codes_to_exclude=['US_nd'])
        expected_query = f"SELECT {self.fake_association_table.name}.column1," \
                         f"{self.fake_association_table.name}.column2," \
                         f"{self.fake_state_table.name}.state_code AS state_code " \
...
class BigQuerySchemaTableRegionFilteredQueryBuilderTest(unittest.TestCase):
    ...
    def test___init__(self) -> None:
        ...
        with self.assertRaises(ValueError):
            BigQuerySchemaTableRegionFilteredQueryBuilder(
                'recidiviz-456', 'my_dataset',
                StateBase, self.fake_jails_table, self.mock_columns_to_include,
                region_codes_to_include=[], region_codes_to_exclude=['US_ID'])

    def test___init__region_codes_none(self) -> None:
        """Test that no assertion is raised if both region_codes_to_include and region_codes_to_exclude are None"""
        BigQuerySchemaTableRegionFilteredQueryBuilder('recidiviz-456', 'my_dataset',
                                                      StateBase, self.fake_jails_table, self.mock_columns_to_include,
                                                      region_codes_to_include=None, region_codes_to_exclude=None)

    def test_select_clause_jails_schema(self) -> None:
        """Given a JailsBase schema, it returns the basic select query for a table."""
        query_builder = BigQuerySchemaTableRegionFilteredQueryBuilder('recidiviz-456', 'my_dataset',
                                                                      JailsBase, self.fake_jails_table,
                                                                      self.mock_columns_to_include)
        expected_select = f'SELECT {self.fake_jails_table.name}.column1,{self.fake_jails_table.name}.state_code'
        self.assertEqual(expected_select, query_builder.select_clause())
    ...
    def test_select_clause_state_schema(self) -> None:
        """Given a StateBase schema, it returns the basic select query for a table."""
        query_builder = BigQuerySchemaTableRegionFilteredQueryBuilder('recidiviz-456', 'my_dataset',
                                                                      StateBase, self.fake_state_table,
                                                                      self.mock_columns_to_include)
        expected_select = f'SELECT {self.fake_state_table.name}.column1,{self.fake_state_table.name}.state_code'
        self.assertEqual(expected_select, query_builder.select_clause())
    ...
    def test_select_clause_state_association_table(self) -> None:
        """Given a StateBase schema and an association table, it includes the state_code in the select statement."""
        query_builder = BigQuerySchemaTableRegionFilteredQueryBuilder('recidiviz-456', 'my_dataset',
                                                                      StateBase, self.fake_association_table,
                                                                      self.mock_association_table_columns_to_include)
        expected_select = f'SELECT {self.fake_association_table.name}.column1,' \
                          f'{self.fake_association_table.name}.column2,' \
