...
from recidiviz.tests.persistence.database.schema.state.schema_test_utils import generate_person, generate_external_id, \
    generate_supervision_violation, generate_supervision_case_type_entry, generate_incarceration_period, \
    generate_supervision_violation_response, generate_bond, generate_parole_decision, \
    generate_early_discharge, generate_program_assignment, generate_supervision_contact
...
class TestStateSchemaUniqueConstraints(unittest.TestCase):
    ...
    generate_functions = [(generate_supervision_violation,), (generate_supervision_case_type_entry,),
                           (generate_incarceration_period,), (generate_supervision_violation_response,),
                           (generate_bond,), (generate_parole_decision,), (generate_early_discharge,),
                           (generate_program_assignment,), (generate_supervision_contact,)]
    # Stores the location of the postgres DB for this test run
    temp_db_dir: Optional[str]
    ...
    def test_add_object_conflicting_external_id(self, generate_func) -> None:
        ...
        db_external_id_duplicated = generate_external_id(
            state_code=self.state_code,
            external_id=self.EXTERNAL_ID_2, id_type=self.ID_TYPE_1)
        db_person = generate_person(
            state_code=self.state_code,
            external_ids=[db_external_id_duplicated])
        db_object_new = generate_func(
    ...
    @parameterized.expand(generate_functions)
    def test_add_object_conflicting_external_id_different_state(self,generate_func) -> None:
        # Arrange
        arrange_session = SessionFactory.for_schema_base(StateBase)
    ...
    @parameterized.expand(generate_functions)
    def test_add_object_conflicting_external_id_same_session(self, generate_func) -> None:
        # Arrange
        db_external_id = generate_external_id(
            state_code=self.state_code,
            external_id=self.EXTERNAL_ID_1, id_type=self.ID_TYPE_1)
...
class TestUniqueExternalIdConstraintOnFine(unittest.TestCase):
    ...
    def test_add_fine_conflicting_external_id_no_flush(self) -> None:
        ...
        db_fine = schema.StateFine(
            person=db_person,
            status=StateFineStatus.EXTERNAL_UNKNOWN.value,
            fine_id=self._ID_1, state_code = self.state_code,
            external_id=self.EXTERNAL_ID_1,
            county_code=self.COUNTY_CODE)
        db_sentence_group = schema.StateSentenceGroup(
            ...
            status=StateSentenceStatus.EXTERNAL_UNKNOWN.value,
            external_id=self.EXTERNAL_ID_1,
            state_code=self.state_code, county_code=self.COUNTY_CODE,
            fines=[db_fine])
        db_external_id = schema.StatePersonExternalId(
            person_external_id_id=self._ID_1, state_code = self.state_code,
            external_id=self.EXTERNAL_ID_1, id_type=self.ID_TYPE_1)
        db_person.sentence_groups = [db_sentence_group]
        db_person.external_ids = [db_external_id]
        ...
        db_fine_dupe = schema.StateFine(
            person=db_person_dupe,
            status=StateFineStatus.EXTERNAL_UNKNOWN.value,
            fine_id=self._ID_2, state_code = self.state_code,
            external_id=self.EXTERNAL_ID_1,
            county_code=self.COUNTY_CODE)
        db_sentence_group_dupe = schema.StateSentenceGroup(
        ...
        state_code=self.state_code, county_code=self.COUNTY_CODE,
            fines=[db_fine_dupe])
        db_external_id_dupe = schema.StatePersonExternalId(
            person_external_id_id=self._ID_2, state_code = self.state_code,
            external_id=self.EXTERNAL_ID_2, id_type=self.ID_TYPE_1)
        db_person_dupe.sentence_groups = [db_sentence_group_dupe]
        db_person_dupe.external_ids = [db_external_id_dupe]
    ...
    def test_add_fine_conflicting_external_id_different_state(self) -> None:
        ...
        db_fine_dupe = schema.StateFine(
            person=db_person_dupe,
            status=StateFineStatus.EXTERNAL_UNKNOWN.value,
            fine_id=self._ID_2, state_code=self.OTHER_STATE_CODE,
            external_id=self.EXTERNAL_ID_1,
            county_code=self.COUNTY_CODE)
        db_sentence_group_dupe = schema.StateSentenceGroup(
            sentence_group_id=self._ID_2,
            status=StateSentenceStatus.EXTERNAL_UNKNOWN.value,
            external_id=self.EXTERNAL_ID_2,
            state_code=self.OTHER_STATE_CODE, county_code=self.COUNTY_CODE,
            fines=[db_fine_dupe])
        db_external_id_dupe = schema.StatePersonExternalId(
            person_external_id_id=self._ID_2, state_code=self.OTHER_STATE_CODE,
            external_id=self.EXTERNAL_ID_2, id_type=self.ID_TYPE_1)
        db_person_dupe.sentence_groups = [db_sentence_group_dupe]
        db_person_dupe.external_ids = [db_external_id_dupe]
