...
class PolicyRequirements:
    ...
    supervision_contact_frequencies: Dict[StateSupervisionCaseType, Dict[StateSupervisionLevel, Tuple[int, int]]]
    ...
    assessment_score_cutoffs: Dict[Gender, Dict[StateSupervisionLevel, Tuple[int, Optional[int]]]]
    ...
    def to_json(self) -> Dict[str, Any]:
        supervision_contact_dict = {
            case_type.value: {
                level.value: period
                for level, period in sub_dict.items()
            }
        ...
        assessment_score_cutoff_dict = {
            gender.value: {
                level.value: score_range
                for level, score_range in sub_dict.items()
            }
            for gender, sub_dict in self.assessment_score_cutoffs.items()
        }

        supervision_level_names = {level.value: text for level, text in self.supervision_level_names.items()}
