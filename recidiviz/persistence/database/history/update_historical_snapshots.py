...
def update_historical_snapshots(session: Session,
    ...
    for snapshot_context in context_registry.all_contexts():
        _write_snapshots(session, snapshot_context, ingest_metadata.ingest_time)
...
def _write_snapshots(session: Session, context: '_SnapshotContext',
    ...
    (This assumes the entity has already been properly updated to reflect that
    it is completed, e.g. by marking a booking as released.)
...
def _write_snapshots_for_new_entities(
    ...
    if provided_start_time and provided_end_time:
        ...
        _post_process_initial_snapshot(context, initial_snapshot)

        session.merge(initial_snapshot)


def _post_process_initial_snapshot(snapshot_context: '_SnapshotContext',
                                   initial_snapshot: Any):
    """Perform any additional processing on the initial closed snapshot, i.e.
    the snapshot representing the state of the entity before its current
    completed state."""
