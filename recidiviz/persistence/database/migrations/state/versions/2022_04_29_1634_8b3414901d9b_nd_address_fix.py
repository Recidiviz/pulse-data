...
Create Date: 2022-04-29 16:34:23.997989
...
REINDEX_QUERY = """REINDEX TABLE state_person;"""


def upgrade() -> None:
    with op.get_context().autocommit_block():
        # Disable indexes while the query updates so that we don't re-index
        # ALL of the state_person indices with each row update.
        # Taken from: https://fle.github.io/temporarily-disable-all-indexes-of-a-postgresql-table.html
        op.execute(DISABLE_INDEXES_QUERY)
        op.execute(UPGRADE_QUERY)
        op.execute(ENABLE_INDEXES_QUERY)
        op.execute(REINDEX_QUERY)
