...
SEX_OFFENDER_QUERY = "SELECT supervision_case_type_entry_id FROM" \
                          " {table_name}" \
                          " WHERE case_type = 'SEX_OFFENDER'"

SEX_OFFENSE_QUERY = "SELECT supervision_case_type_entry_id FROM" \
                          " {table_name}" \
                          " WHERE case_type = 'SEX_OFFENSE'"

UPDATE_QUERY = "UPDATE {table_name} SET case_type = '{new_value}'" \
...
def upgrade():
    ...
    connection.execute(UPDATE_QUERY.format(table_name=CASE_TYPE_TABLE_NAME,
                                           new_value=new_sex_offense,
                                           ids_query=SEX_OFFENDER_QUERY.format(table_name=CASE_TYPE_TABLE_NAME)))

    connection.execute(UPDATE_QUERY.format(table_name=CASE_TYPE_HISTORY_TABLE_NAME,
                                           new_value=new_sex_offense,
