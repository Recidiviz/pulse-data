...
def read_people_by_cls_external_ids(
    ...
    logging.info(
        "[DAO] Starting read of external ids of class [%s]", schema_cls.__name__
    )
    person_ids_result = (
        session.query(schema_cls.person_id)
...
def read_placeholder_persons(
    ...
    person_ids_result = (
        session.query(schema.StatePerson.person_id)
        .outerjoin(schema.StatePersonExternalId)
        .filter(schema.StatePerson.state_code == state_code.upper())
        .filter(schema.StatePersonExternalId.external_id.is_(None))
        .all()
    )
    person_ids = [res[0] for res in person_ids_result]
    logging.info(
...
def read_external_ids_of_cls_with_external_id_match(
    ...
    external_ids_by_count = (
        ...
        .group_by(external_ids_subquery.c.external_id)
        .subquery()
    )

    external_ids_result = (
        session.query(external_ids_by_count.c.external_id)
        .filter(external_ids_by_count.c.cnt > 1)
        .all()
    )
...
def read_people_by_external_ids(
    session: Session, _region: str, ingested_people: List[entities.StatePerson]
) -> List[schema.StatePerson]:
    """
