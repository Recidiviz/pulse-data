...
def monitor_refresh_bq_tasks() -> Tuple[str, int]:
    ...
    lock_manager.unlock(POSTGRES_TO_BQ_EXPORT_RUNNING_LOCK_NAME)
    logging.info('Done running, unlocking Postgres to BigQuery export')
...
def wait_for_ingest_to_create_tasks(schema_arg: str) -> Tuple[str, HTTPStatus]:
    ...
    if not lock_manager.is_locked(POSTGRES_TO_BQ_EXPORT_RUNNING_LOCK_NAME):
        time = datetime.now().strftime("%m/%d/%Y, %H:%M:%S")
        contents_as_json = {"time": time, "lock_id": lock_id}
        contents = json.dumps(contents_as_json)
        lock_manager.lock(POSTGRES_TO_BQ_EXPORT_RUNNING_LOCK_NAME, contents)
    else:
        contents = lock_manager.get_lock_contents(POSTGRES_TO_BQ_EXPORT_RUNNING_LOCK_NAME)
        try:
    ...
    if not no_regions_running:
        ...
        task_manager.job_monitor_cloud_task_queue_manager.create_task(
            task_id=task_id,
            body=body,
            relative_uri=f'/cloud_sql_to_bq/wait_to_create_refresh_bq_tasks/{schema_arg}',
