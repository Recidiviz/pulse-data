...
def refresh_bq_table_from_gcs_export_synchronous(
    ...
    logging.info(
        "Loading BQ Table [%s] from temp table [%s]", table_name, temp_table_name
    )
    ...
    delete_temp_table_if_exists(
        big_query_client, temp_table_name, cloud_sql_to_bq_config
    )
...
def load_rows_excluded_from_refresh_into_temp_table_and_wait(
    ...
    table_refresh_query_builder = (
        cloud_sql_to_bq_config.get_stale_bq_rows_for_excluded_regions_query_builder(
            table_name
        )
    )
...
def delete_temp_table_if_exists(
    ...
    dataset_ref = cloud_sql_to_bq_config.get_dataset_ref(big_query_client)
    if not big_query_client.table_exists(
        dataset_ref=dataset_ref, table_id=temp_table_name
    ):
        logging.info(
            "Delete temp table failed, table [%s] does not exist.", temp_table_name
        )
        return
    big_query_client.delete_table(
        dataset_id=cloud_sql_to_bq_config.dataset_id, table_id=temp_table_name
    )
