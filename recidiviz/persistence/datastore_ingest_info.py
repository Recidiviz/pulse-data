...
_DatastoreIngestInfoType = TypeVar(
    "_DatastoreIngestInfoType", bound="_DatastoreIngestInfo"
)
...
class _DatastoreIngestInfo:
    ...
    @classmethod
    def get_batch_ingest_info_data(
        cls, entity: datastore.Entity
    ) -> BatchIngestInfoData:
        batch_ingest_info_data_serialized = cls(entity).__dict__["_entity"][
            "batch_ingest_info_data"
        ]
    ...
    @classmethod
    def new(
        cls: Type[_DatastoreIngestInfoType],
        key: datastore.key,
        task_hash: int,
    ...
    ) -> _DatastoreIngestInfoType:
        new_ingest_info = cls(datastore.Entity(key))
        batch_ingest_info_data = BatchIngestInfoData(
            task_hash=task_hash, ingest_info=ingest_info, error=error, trace_id=trace_id
        )
        # pylint: disable=protected-access
        new_ingest_info._entity["region"] = region
        new_ingest_info._entity[
            "batch_ingest_info_data"
...
def batch_get_ingest_infos_for_region(
    region: str, session_start_time: datetime
...
def _divide_into_chunks(
    results: List[datastore.Entity], chunk_size: int
...
def _batch_ingest_info_data_from_entities(
    entity_list: List[datastore.Entity],
) -> List[BatchIngestInfoData]:
    return [
        _DatastoreIngestInfo.get_batch_ingest_info_data(entity)
