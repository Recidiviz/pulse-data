        ...
        for sentence_group in person.sentence_groups:
            for sentence in sentence_group.supervision_sentences + sentence_group.incarceration_sentences:
                unmatched_svs = _move_events_onto_supervision_periods(
                    sentence, schema.StateSupervisionViolation, 'supervision_violation_entries')
...
def _move_events_onto_supervision_periods(
        source: DatabaseEntity, event_cls: Type[DatabaseEntity], event_field_name: str) -> List[DatabaseEntity]:
...
def _get_event_date(source: DatabaseEntity) -> Optional[datetime.date]:
    ...
    raise ValueError(f'Unexpected event entity found of type [{source.get_entity_name()}]')
