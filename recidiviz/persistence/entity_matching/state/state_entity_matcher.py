...
class StateEntityMatcher(BaseEntityMatcher[entities.StatePerson]):
    ...
    def run_match(self, session: Session, region: str,
        ...
        with session.no_autoflush:
            ...
            matched_entities = _run_match(ingested_db_people, db_persons)
...
def _run_match(ingested_persons: List[schema.StatePerson],
...
def _perform_preprocessing(ingested_persons: List[schema.StatePerson]):
...
def _match_persons(
    ...
    _populate_person_backedges(updated_persons)
...
def _populate_person_backedges(updated_persons: List[schema.StatePerson]):
...
def _match_placeholder_tree(
    ...
    def resolve_child_placeholder_match_result():
        ...
        if not child_match_result.merged_entity_trees:
            raise EntityMatchingError(
                f"Expected child_match_result.merged_entity_trees to not be "
                f"empty.", ingested_placeholder_tree.entity.get_entity_name())
    ...
    db_placeholder = shallow_db_copy(ingested_placeholder_tree.entity)
    ...
    return IndividualMatchResult(merged_entity_trees=updated_entity_trees,
                                 error_count=error_count)

def shallow_db_copy(old_entity: DatabaseEntity) -> DatabaseEntity:
    ent_cls = old_entity.__class__
    new_entity = ent_cls()
    for field in get_all_core_entity_field_names(old_entity,
...
def _match_unmatched_tree(
    ...
    new_db_entity = shallow_db_copy(ingested_unmatched_entity_tree.entity)
    for child_field_name, match_results in match_results_by_child:
        error_count += match_results.error_count
...
def _match_matched_tree(
    ...
    return IndividualMatchResult(merged_entity_trees=[merged_entity_tree],
                                 error_count=error_count)
...
def _get_match(
    ...
    if not exact_match:
        ...
        if isinstance(ingested_entity_tree.entity,
                      (schema.StateAgent, schema.StateIncarcerationSentence,
                       schema.StateAssessment, schema.StateSupervisionPeriod,
...
def merge_multiparent_entities(persons: List[schema.StatePerson]):
    ...
    for person in persons:
        if not isinstance(person, schema.StatePerson):
            raise ValueError("Expected person to have type schema.StatePerson")

        charge_map: Dict[str, List[_EntityWithParents]] = {}
        _populate_multiparent_map(person, schema.StateCharge, charge_map)
        _merge_multiparent_entities_from_map(charge_map)

        court_case_map: Dict[str, List[_EntityWithParents]] = {}
        _populate_multiparent_map(person, schema.StateCourtCase, court_case_map)
        _merge_multiparent_entities_from_map(court_case_map)

        agent_map: Dict[str, List[_EntityWithParents]] = {}
        _populate_multiparent_map(person, schema.StateAgent, agent_map)
...
def _is_subset(entity: DatabaseEntity, subset: DatabaseEntity) -> bool:
...
def _replace_entity(
        *, entity: DatabaseEntity, to_replace: DatabaseEntity,
        linked_parents: List[_LinkedParents]):
