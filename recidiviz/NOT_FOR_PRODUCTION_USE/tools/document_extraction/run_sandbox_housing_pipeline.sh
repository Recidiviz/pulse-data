#!/bin/bash
# Runs the full housing info extraction pipeline in a sandbox.
#
# Stages (comment out any stage to skip it):
#   1a. Doc store refresh         (US_IX_SUPERVISION_CASE_NOTES)
#   1b. Extraction + view deploy  (US_IX_CASE_NOTE_HOUSING_INFO)
#   2a. ER doc store refresh      (US_IX_CASE_NOTE_HOUSING_CONTEXT)
#   2b. ER extraction + views     (US_IX_CASE_NOTE_HOUSING_ENTITY_RESOLUTION)
#   3.  Summary views             (housing sessions + current_housing_status_summary)
#
# Usage: edit the variables below, then run:
#   bash recidiviz/NOT_FOR_PRODUCTION_USE/tools/document_extraction/run_sandbox_housing_pipeline.sh
...
ER_DOC_COLLECTION_NAME="US_IX_CASE_NOTE_HOUSING_CONTEXT"
ER_EXTRACTOR_ID="US_IX_CASE_NOTE_HOUSING_ENTITY_RESOLUTION"
...
sep "STAGE 3: Housing summary views"
python -m "${SUMMARY}" \
  --project_id "${PROJECT_ID}" \
  --sandbox_dataset_prefix "${SANDBOX_PREFIX}" \
