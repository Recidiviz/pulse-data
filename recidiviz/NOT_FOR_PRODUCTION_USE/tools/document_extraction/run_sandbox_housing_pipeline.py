...
# =============================================================================
"""Housing extraction pipeline with phase control and segmentation.

Replaces run_sandbox_housing_pipeline.sh with a Python script that supports
resumable, parallelizable extraction of up to 1M+ documents.

Phases:
  HOUSING_DOC_UPLOAD         Upload case note documents to GCS
  HOUSING_EXTRACTION         Extract housing info from case notes
  HOUSING_VIEW_DEPLOY        Deploy extraction views for housing extractor
  HOUSING_ER_DOC_UPLOAD      Upload housing context docs (derived from extraction)
  HOUSING_ER_EXTRACTION      Run housing entity resolution
  HOUSING_ER_VIEW_DEPLOY     Deploy extraction views for ER extractor
  SUMMARY_VIEW_DEPLOY        Deploy summary views + set table expiration

Usage:
    # Check status of all phases:
    python -m recidiviz.NOT_FOR_PRODUCTION_USE.tools.document_extraction.run_sandbox_housing_pipeline \
        --project_id recidiviz-staging \
        --state_code US_IX \
        --sandbox_dataset_prefix anna_housing \
    ...
    python -m recidiviz.NOT_FOR_PRODUCTION_USE.tools.document_extraction.run_sandbox_housing_pipeline \
        ...
        --sandbox_dataset_prefix anna_housing \
        --sandbox_documents_bucket recidiviz-staging-anna-scratch \
        --phase HOUSING_EXTRACTION --segment_index 0 --total_segments 10
"""
import argparse
import datetime
import enum
...
_PRIMARY_EXTRACTOR_SUFFIX = "CASE_NOTE_HOUSING_INFO"
_ER_EXTRACTOR_SUFFIX = "CASE_NOTE_HOUSING_ENTITY_RESOLUTION"
...
class HousingPhase(enum.Enum):
    HOUSING_DOC_UPLOAD = "HOUSING_DOC_UPLOAD"
    HOUSING_EXTRACTION = "HOUSING_EXTRACTION"
    HOUSING_VIEW_DEPLOY = "HOUSING_VIEW_DEPLOY"
    HOUSING_ER_DOC_UPLOAD = "HOUSING_ER_DOC_UPLOAD"
    HOUSING_ER_EXTRACTION = "HOUSING_ER_EXTRACTION"
    HOUSING_ER_VIEW_DEPLOY = "HOUSING_ER_VIEW_DEPLOY"
    SUMMARY_VIEW_DEPLOY = "SUMMARY_VIEW_DEPLOY"


def _get_phase_config(
    state_code: str,
) -> dict[HousingPhase, tuple[str, str]]:
    ...
    return {
        HousingPhase.HOUSING_DOC_UPLOAD: (primary, "DOC_UPLOAD"),
        HousingPhase.HOUSING_EXTRACTION: (primary, "EXTRACTION"),
        HousingPhase.HOUSING_VIEW_DEPLOY: (primary, "VIEW_DEPLOY"),
        HousingPhase.HOUSING_ER_DOC_UPLOAD: (er, "DOC_UPLOAD"),
        HousingPhase.HOUSING_ER_EXTRACTION: (er, "EXTRACTION"),
        HousingPhase.HOUSING_ER_VIEW_DEPLOY: (er, "VIEW_DEPLOY"),
        HousingPhase.SUMMARY_VIEW_DEPLOY: (primary, "SUMMARY_VIEW_DEPLOY"),
    }
...
def show_status(
    ...
    """Prints completion status of all housing pipeline phases."""
    bq_client = BigQueryClientImpl()
    ...
    print(f"  HOUSING_DOC_UPLOAD         {_fmt(upload_status, upload_count)}")
    print(
        f"  HOUSING_EXTRACTION         "
        f"{_fmt(extraction_status, extraction_count, expected_extraction_docs)}"
    )
    print(f"  HOUSING_VIEW_DEPLOY        {ready}")
    print(f"  HOUSING_ER_DOC_UPLOAD      {_fmt(er_upload_status, er_upload_count)}")
    print(
        f"  HOUSING_ER_EXTRACTION      "
        f"{_fmt(er_extraction_status, er_extraction_count, expected_er_docs)}"
    )
    print(f"  HOUSING_ER_VIEW_DEPLOY     {ready}")
...
def run_phase(  # pylint: disable=too-many-positional-arguments
    ...
    """Runs a single housing pipeline phase."""
    ...
    if phase_type == "DOC_UPLOAD":
        ...
        builders = [b for b in all_builders if "case_note_housing_info" in b.view_id]
        if builders:
            logging.info(
...
def parse_arguments(argv: list[str]) -> argparse.Namespace:
    """Parses command-line arguments for the housing pipeline."""
    parser = argparse.ArgumentParser(
        description="Housing extraction pipeline with phase control."
    )
    parser.add_argument(
        "--project_id",
        type=str,
    ...
    parser.add_argument(
        "--phase",
        type=str,
        choices=[p.value for p in HousingPhase],
        default=None,
...
if __name__ == "__main__":
    ...
    with local_project_id_override(args.project_id):
        if args.phase is None:
            ...
            parsed_phase = HousingPhase(args.phase)
            run_phase(
                phase=parsed_phase,
