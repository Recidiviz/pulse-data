#!/bin/bash
# Runs the full employment info extraction pipeline in a sandbox.
#
# Stages (comment out any stage to skip it):
#   1a. Doc store refresh         (US_IX_SUPERVISION_CASE_NOTES)
#   1b. Extraction + view deploy  (US_IX_CASE_NOTE_EMPLOYMENT_INFO)
#   2a. ER doc store refresh      (US_IX_CASE_NOTE_EMPLOYER_CONTEXT)
#   2b. ER extraction + views     (US_IX_CASE_NOTE_EMPLOYER_ENTITY_RESOLUTION)
#   3.  Summary views             (employment sessions + current_employment_status_summary)
#
# Usage: edit the variables below, then run:
#   bash recidiviz/NOT_FOR_PRODUCTION_USE/tools/document_extraction/run_sandbox_employment_pipeline.sh
...
ER_DOC_COLLECTION_NAME="US_IX_CASE_NOTE_EMPLOYER_CONTEXT"
ER_EXTRACTOR_ID="US_IX_CASE_NOTE_EMPLOYER_ENTITY_RESOLUTION"
...
sep "STAGE 2a: ER doc store refresh (${ER_DOC_COLLECTION_NAME})"
python -m "${DOC_REFRESH}" \
  --project_id "${PROJECT_ID}" \
  --collection_name "${ER_DOC_COLLECTION_NAME}" \
  --sandbox_dataset_prefix "${SANDBOX_PREFIX}" \
  --sandbox_bucket "${SANDBOX_BUCKET}" \
  "${SAMPLE_FLAGS[@]}"

sep "STAGE 2b: ER extraction (${ER_EXTRACTOR_ID})"
python -m "${EXTRACTION}" \
  --project_id "${PROJECT_ID}" \
  --extractor_id "${ER_EXTRACTOR_ID}" \
  --sandbox_dataset_prefix "${SANDBOX_PREFIX}" \
  --sandbox_documents_bucket "${SANDBOX_BUCKET}" \
  "${SAMPLE_FLAGS[@]}" \
  "${MODE_FLAGS[@]}"

sep "STAGE 3: Employment summary views"
python -m "${SUMMARY}" \
  --project_id "${PROJECT_ID}" \
  --sandbox_dataset_prefix "${SANDBOX_PREFIX}" \
...
sep "Employment pipeline complete!"
