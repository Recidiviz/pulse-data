...
# =============================================================================
"""Defines a view that shows supervision level downgrades after the initial
classification review date has passed (ie. after 4-12 months of supervision depending on the case).
"""
...
_DESCRIPTION = """Defines a view that shows supervision level downgrades after the initial
classification review date has passed (ie. after 4-12 months of supervision depending on the case).
"""
...
LEFT JOIN `{{project_id}}.{{criteria_dataset}}.past_initial_classification_review_date_materialized` tes
...
WHERE sls.supervision_downgrade = 1
    ...
    --only take rows where a client is past their initial review date (meets_criteria = TRUE), or they do not have a 
    --span in the tes view (meets_criteria IS NULL). they won't have a span after their intial classification review,
     --so all rows with null meets_criteria should be counted as slds after their initial review 
    AND COALESCE(tes.meets_criteria, TRUE)
"""
...
VIEW_BUILDER: StateSpecificTaskCompletionEventBigQueryViewBuilder = StateSpecificTaskCompletionEventBigQueryViewBuilder(
    ...
    completion_event_type=TaskCompletionEventType.SUPERVISION_LEVEL_DOWNGRADE_AFTER_INITIAL_CLASSIFICATION_REVIEW_DATE,
