...
def state_agnostic_candidate_population_query(
    *, additional_filters: List[str], compartment_level_1: str
) -> str:
    """Returns a query with the candidate population as tracked by `dataflow_sessions`
    with any additional filter conditions applied to the query WHERE clause."""

    if compartment_level_1 not in {"INCARCERATION", "SUPERVISION"}:
        raise ValueError(f"Unsupported compartment_level_1 value {compartment_level_1}")
