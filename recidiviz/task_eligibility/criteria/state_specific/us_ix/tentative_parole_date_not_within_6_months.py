...
_CRITERIA_NAME = "US_IX_TENTATIVE_PAROLE_DATE_NOT_WITHIN_6_MONTHS"
...
_QUERY_TEMPLATE = f"""
WITH tpd AS (
    -- Spans of time where TPD is within 6 months
    SELECT 
        state_code,
        person_id,
        DATE_SUB(tentative_parole_date, INTERVAL 6 MONTH) AS start_date,
        DATE_ADD(tentative_parole_date, INTERVAL 1 DAY) AS end_date,
        tentative_parole_date,
    FROM `{{project_id}}.{{analyst_dataset}}.us_ix_parole_dates_spans_preprocessing_materialized`
    WHERE tentative_parole_date IS NOT NULL
),
{create_sub_sessions_with_attributes('tpd')}
    SELECT
      state_code,
      person_id,
      start_date,
