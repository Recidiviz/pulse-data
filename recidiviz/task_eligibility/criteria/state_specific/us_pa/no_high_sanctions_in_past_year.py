...
WITH sanctions_sessions_cte AS (
        SELECT
          v.person_id,
          v.state_code,
          CAST(violation_date AS DATE) AS start_date,
          DATE_ADD(CAST(violation_date AS DATE), INTERVAL 12 MONTH) AS end_date,
          CAST(violation_date AS DATE) AS latest_high_sanction_date,
