...
# ============================================================================
"""Defines a criteria span view that shows spans of time during which one year has passed
from the last security classification committee review where the ADD was in person.
"""
...
_CRITERIA_NAME = "US_MI_ONE_YEAR_PAST_LAST_ADD_IN_PERSON_SECURITY_CLASSIFICATION_COMMITTEE_REVIEW_DATE"

_DESCRIPTION = """Defines a criteria span view that shows spans of time during which one year has passed
from the last security classification committee (SCC) review where the ADD was in person. 
"""
...
SELECT 
    ...
    DATE_ADD(completion_event_date, INTERVAL 1 YEAR) AS critical_date
FROM `{{project_id}}.{{analyst_views_dataset}}.us_mi_add_in_person_security_classification_committee_review_materialized` ce
