...
def make_config_directory(state_code: str) -> str:
    return os.path.join(os.path.split(os.path.dirname(__file__))[0],
                        'ingest', 'direct', 'regions', state_code, 'raw_data')
...
def initialize_state_directories(state_code: str) -> None:
    ...
    if os.path.exists(base_config_path):
        logging.error('Default config file %s already exists. Cannot initialize state.', base_config_path)
        sys.exit(1)

    with open(base_config_path, "w") as file:
        file.writelines([f'default_encoding: {PLACEHOLDER}\n',
                         f'default_separator: {PLACEHOLDER}\n'])


def write_skeleton_config(raw_table_path: str, state_code: str, delimiter: str, allow_overwrite: bool) -> None:
    ...
    if len(fields) == 1:
        logging.error("Unable to split header of %s on delimiter '%s'", raw_table_path, delimiter)
        return
    ...
    if not allow_overwrite and os.path.exists(config_path):
        logging.info('File %s already exists. Skipping skeleton generation', config_path)
        return
...
def create_ingest_config_skeleton(
    ...
    if initialize_state:
        ...
        logging.error('Folder %s does not exist. Run with --initialize-state to create directory structure.',
...
def parse_arguments(argv: List[str]) -> argparse.Namespace:
    ...
    file_group.add_argument('--folder',
                            dest='folder_path',
    ...
    parser.add_argument('--initialize-state',
...
if __name__ == '__main__':
    ...
    if args.file_path:
        create_ingest_config_skeleton([args.file_path],
                                      args.state_code,
                                      args.delimiter,
        ...
        create_ingest_config_skeleton([f'{args.folder_path}/{filename}' for filename in os.listdir(args.folder_path)],
                                      args.state_code,
                                      args.delimiter,
                                      args.allow_overwrite,
                                      args.initialize_state)
