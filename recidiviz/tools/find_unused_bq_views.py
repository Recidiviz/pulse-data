...
UNREFERENCED_ADDRESSES_TO_KEEP_WITH_REASON: Dict[BigQueryAddress, str] = {
    ...
    **{
        ...
        for table_id in [
            ...
            "cni_accuracy_per_field_annotation_overrides",
            ...
            "meetings_module_quality_annotation_overrides",
    ...
    **{
    ...
    BigQueryAddress.from_str("segment_events.all_meetings_segment_events"): (
        "Union view for meetings segment events. Kept for potential future use. "
        "(Mayuka Sarukkai, 2026-04-15)"
    ),
    ...
    **{
    ...
    BigQueryAddress.from_str(
        ...
        "(Feona Dong 2026-08-11)"
    ),
    BigQueryAddress.from_str(
        "task_eligibility_criteria_general.no_supervision_violation_within_30_days"
    ): (
        "View not yet referenced, to be included in TES criteria for EM removal. "
        "(Feona Dong 2026-08-14)"
    ),
    BigQueryAddress.from_str(
        "task_eligibility_criteria_general.at_least_60_days_since_negative_drug_test_streak_began"
    ): (
        "View not yet referenced, to be included in TES criteria for EM removal. "
...
def _should_ignore_unused_address(address: BigQueryAddress) -> bool:
    ...
    if address == BigQueryAddress(
        dataset_id=CLASSIFICATION_VIEWS_DATASET,
        table_id=ALL_CLASSIFICATION_SCORE_COMPONENTS_VIEW_ID,
    ):
        # Autogenerated union of every classification score component span view, and the
