...
class MoveFilesFromUnspecifiedToRawController:
    ...
    def run(self) -> None:
        ...
        if self.dry_run:
            logging.info("[DRY RUN] Found [%d] subdirectories to move",
                         len(subdirs_to_move))
    ...
    def _move_files_for_date(self, subdir_path_str: str) -> None:
        ...
        from_dir_path = GcsfsDirectoryPath.from_absolute_path(subdir_path_str.rstrip('/'))
        ...
        for from_path in from_paths:
            file_name = GcsfsFilePath(bucket_name=self.region_storage_dir_path.bucket_name,
                                      blob_name=from_path).file_name

            to_file_path = os.path.join('gs://', self.region_storage_dir_path.bucket_name,
                                        self.region_code,
    ...
    def _execute_move(self, subdirs_to_move: List[str]) -> None:
        self.move_progress = Bar("Moving files from subdirectories...", max=len(subdirs_to_move))
