...
# =============================================================================
"""Creates a new supplemental_data dataset prefixed with the provided sandbox_dataset_prefix,
where the schema of the supplemental dataset tables will match the attributes on the
supplemental dataset classes locally. This script is used to create a test output
location when testing Dataflow pipeline changes. The sandbox_dataset_prefix provided
should be your github username or some personal unique identifier so it's easy for others
to tell who created the dataset.

Run locally with the following command:

    python -m recidiviz.tools.calculator.creat_or_update_supplemental_datasets_sandbox \
...
from recidiviz.calculator.pipeline.supplemental import dataset_config
...
def create_or_update_supplemental_datasets_sandbox(
    sandbox_dataset_prefix: str, allow_overwrite: bool = False
) -> None:
    """Creates or updates a supplemental dataset sandbox dataset, prefixing the dataset
    name with the given prefix."""
    sandbox_dataset_id = (
        sandbox_dataset_prefix + "_" + dataset_config.SUPPLEMENTAL_DATA_DATASET
    )
...
if __name__ == "__main__":
    ...
    with local_project_id_override(known_args.project_id):
        create_or_update_supplemental_datasets_sandbox(
            known_args.sandbox_dataset_prefix, known_args.allow_overwrite
        )
