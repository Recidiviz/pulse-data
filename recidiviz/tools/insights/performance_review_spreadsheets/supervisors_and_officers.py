...
WITH current_ix_officers as (
    SELECT external_id AS staff_external_id, state_code, full_name, supervision_district, supervisor_external_ids, email, from `recidiviz-123.outliers_views.supervision_officers_materialized` where state_code="US_IX" and ARRAY_LENGTH(supervisor_external_ids)!=0
),
midyear_officers as(
  Select soa.staff_external_id, soa.state_code, soa.full_name, soa.supervision_district, soa.supervisor_external_ids, string(NULL) as email from `recidiviz-123.outliers_views.supervision_officers_archive_materialized` soa left join current_ix_officers c on c.staff_external_id=soa.staff_external_id where soa.state_code="US_IX" and export_date="2024-07-01" and c.staff_external_id is NULL and ARRAY_LENGTH(soa.supervisor_external_ids)!=0
),
september_officers as(
    ...
    where soa.state_code="US_IX" and export_date="2024-09-01" and c.staff_external_id is NULL and mid.external_id is NULL and ARRAY_LENGTH(soa.supervisor_external_ids)!=0
),
