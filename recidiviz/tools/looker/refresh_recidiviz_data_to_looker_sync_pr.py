...
class LookerGitManager:
    ...
    config: RecidivizDataLookerSyncConfig = attr.ib(
    ...
    def clone_and_checkout_branch(self) -> None:
        ...
        looker_repo_url = (
            f"https://{self.config.github_token}@github.com/{LOOKER_REPO_NAME}.git"
        )
        ...
        if self._remote_branch_exists(self.config.looker_branch_name):
    ...
    def commit_and_push_changes(self) -> None:
        ...
        self._run_in_looker_repo(
            f'git commit -am "{AUTOGENERATED_MESSAGE_PREFIX}@{self.config.recidiviz_data_commit_sha}"'
        )

        if self._remote_branch_exists(self.config.looker_branch_name):
            cmd = f"git push origin {self.config.looker_branch_name}"
        else:
            cmd = f"git push --set-upstream origin {self.config.looker_branch_name}"
...
class RecidivizDataLookerSyncGitHubManager:
    ...
    next_steps_body = (
        ...
        "the `check_looker_generated_versions_match` workflow will fail on merge."
    )
    ...
    def comment_on_recidiviz_data_pr(self, looker_pr_url: str) -> None:
        """Add a comment to the original data PR with instructions."""
        comment_body = (
            f"## Looker repo PR\n"
...
class RecidivizDataLookerSyncOrchestrator:
    ...
    looker_git_manager: LookerGitManager = attr.ib(
        validator=attr.validators.instance_of(LookerGitManager)
    )
    ...
    def refresh_recidiviz_data_to_looker_sync_pr(self) -> None:
        ...
        self.looker_git_manager.clone_and_checkout_branch()

        copy_all_autogenerated_lookml(str(self.looker_git_manager.looker_repo_root))

        if not self.looker_git_manager.has_changes():
            logging.info("No changes to LookML files. Exiting.")
            return

        self.looker_git_manager.commit_and_push_changes()

        looker_pr_url = self.github_manager.create_looker_pr_if_not_exists()
        self.github_manager.comment_on_recidiviz_data_pr(looker_pr_url)
