...
import {
  to = module.data-platform-airflow-pagerduty-service.pagerduty_service_integration.email_integration
  id = "P1A7TVN.PFGP7NF"
...
import {
  to = module.monitoring-airflow-pagerduty-service.pagerduty_service_integration.email_integration
  id = "P279952.PF953FR"
  # Only import if project is staging, otherwise create a new integration.
  for_each = var.project_id == "recidiviz-staging" ? [0] : []
}
...
module "data-platform-airflow-pagerduty-service" {
  source = "./modules/pagerduty-service-airflow"
  providers = {
    pagerduty = pagerduty
  }
...
module "monitoring-airflow-pagerduty-service" {
  ...
  project_id           = var.project_id
  service_base_name    = "Airflow Tasks: Monitoring"
  service_description  = "Airflow tasks responsible for generating PagerDuty alerts."
