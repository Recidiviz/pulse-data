...
locals {
  # This project's principals, from its Secret Manager secret. `secret_data` is
  # marked sensitive, and Terraform forbids sensitive values as `for_each` keys
  # (they would surface as resource instance keys). These are AWS account IDs and
  # role ARNs — identifiers, not credentials, and they already appear in plan
  # output — so `nonsensitive` is safe here; Secret Manager is used to keep them
  # out of the repo, not because they are cryptographic secrets.
  edovo_aws_principals = yamldecode(
    nonsensitive(data.google_secret_manager_secret_version.edovo_aws_principals.secret_data)
  )
