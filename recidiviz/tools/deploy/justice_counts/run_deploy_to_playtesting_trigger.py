...
# =============================================================================
"""
Script to run the Deploy-to-playtesting Cloud Build Trigger, which builds a Docker image
containing both Publisher and Agency Dashboard and then deploys it to a playtest URL.
Generally meant to be called from the Justice Counts deploy_for_playtesting script.
You need to specify:
...
- service name (either publisher-web or agency-dashboard-web)
- playtesting URL tag

Example usage (run from `pipenv shell`):
python -m recidiviz.tools.deploy.justice_counts.run_deploy_to_playtesting_trigger \
    --backend-tag jc.publisher.v1.0.0 \
    --frontend-tag publisher.v1.0.0 \
    --subdirectory main \
    --service-name publisher-web \
    --url-tag test

python -m recidiviz.tools.deploy.justice_counts.run_deploy_to_playtesting_trigger \
    --backend-branch main \
    --frontend-branch-or-sha main \
...
CLOUD_BUILD_TRIGGER = "Deploy-to-playtesting"
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--service-name",
        help="Name of the service to run from the playtest link.",
        choices=["publisher-web", "agency-dashboard-web"],
        required=True,
    )

    parser.add_argument(
        "--url-tag",
        help="Name of the playtesting URL.",
        required=True,
    )
...
def main(
    ...
    """Run the Justice Counts Deploy-to-playtesting Cloud Build Trigger."""
