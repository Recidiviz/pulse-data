...
from recidiviz.tools.utils import (
    to_datetime,
    add_row_to_raw_metadata,
    get_file_id_and_processed_status_for_file,
    mark_existing_metadata_row_as_processed,
    MetadataType,
    get_next_available_file_id,
)
...
class UploadRawDataFilesToBqController:
    ...
    def _append_df_to_table(
        self, dataset: str, table_name: str, df: pd.DataFrame
    ) -> None:
        ...
        if self.dry_run:
            logging.info(
                "[DRY RUN] Would have uploaded dataframe to %s.%s.%s",
                self.project_id,
                dataset,
                table_name,
            )
            return
        logging.info(
            "Uploading dataframe to to %s.%s.%s", self.project_id, dataset, table_name
        )
    ...
    def _get_dataframe_from_csv_with_extra_cols(
        self, file_id: int, local_file_path: str
    ) -> pd.DataFrame:
    ...
    def _upload_raw_data_file_to_bq(self, local_file_path: str) -> None:
        ...
        file_id = (
            file_id_processed_tuple[0]
            if file_id_processed_tuple[0] is not None
    ...
    def do_upload(self) -> None:
        ...
        for local_file_path in local_file_paths:
            try:
                self._upload_raw_data_file_to_bq(local_file_path=local_file_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        default="",
        help="The region code for the uploaded file(s)",
    )
    parser.add_argument(
        "--import-time",
        required=True,
        help="The time the file was received by Recidiviz. Expected in "
        "format %Y-%m-%d %H:%M",
    )
    parser.add_argument(
        "--separator",
