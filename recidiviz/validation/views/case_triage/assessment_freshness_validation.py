...
ASSESSMENT_FRESHNESS_FRAGMENT = (
    f"{current_date_sub(days=1)} AND {current_date_add(days=100)}"
)

UPDT_DT_CLAUSE = "SAFE_CAST(SPLIT(updt_dt, ' ')[OFFSET(0)] AS DATE)"
TST_DT_CLAUSE = "SAFE_CAST(SPLIT(tst_dt, ' ')[OFFSET(0)] AS DATE)"
...
ASSESSMENT_FRESHNESS_VALIDATION_VIEW_BUILDER = FreshnessValidation(
    ...
    assertions=[
        FreshnessValidationAssertion.build_assertion(
            ...
            table="ofndr_tst",
            freshness_clause=f"CAST(update_datetime AS DATE) BETWEEN {ASSESSMENT_FRESHNESS_FRAGMENT}",
        ),
        FreshnessValidationAssertion.build_assertion(
            region_code="US_ID",
            ...
            dataset="us_id_raw_data",
            table="ofndr_tst",
            freshness_clause=f"{UPDT_DT_CLAUSE} BETWEEN {ASSESSMENT_FRESHNESS_FRAGMENT}",
        ),
        FreshnessValidationAssertion.build_assertion(
            region_code="US_ID",
            ...
            dataset="us_id_raw_data",
            table="ofndr_tst",
            freshness_clause=f"{TST_DT_CLAUSE} BETWEEN {ASSESSMENT_FRESHNESS_FRAGMENT}",
        ),
        FreshnessValidationAssertion.build_assertion(
            region_code="US_ID",
            ...
            freshness_clause=f"state_code = 'US_ID' AND assessment_date BETWEEN {ASSESSMENT_FRESHNESS_FRAGMENT}",
        ),
        FreshnessValidationAssertion.build_assertion(
            region_code="US_ID",
            ...
            freshness_clause=f"state_code = 'US_ID' AND most_recent_assessment_date BETWEEN {ASSESSMENT_FRESHNESS_FRAGMENT}",
        ),
    ],
