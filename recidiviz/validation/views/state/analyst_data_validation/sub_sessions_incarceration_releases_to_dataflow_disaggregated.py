...
SUB_SESSIONS_INCARCERATION_RELEASES_TO_DATAFLOW_DISAGGREGATED_DESCRIPTION = """
    ...
    sub-sessions that do not have any dataflow end reason associated with them. The second three (dataflow_release, 
    sub_session_release, session_release) are used to identify dataflow releases that are not represented in the 
    sessions or sub-sessions views. Note that a subset of release reasons is used in this comparison because of the fact
    that we would not expect every dataflow metric event to be associated with a compartment transition.
    """
