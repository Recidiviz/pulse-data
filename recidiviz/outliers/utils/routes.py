...
from recidiviz.metrics.export.export_config import OUTLIERS_VIEWS_OUTPUT_DIRECTORY_URI
...
def get_outliers_utils_blueprint() -> Blueprint:
    ...
    # This endpoint is triggered by a pub/sub subscription on the outliers-etl-data-archive GCS bucket.
    # To trigger it manually, run (substituting PROJECT_ID and FILENAME):
    # `gcloud pubsub topics publish storage-notification-$PROJECT_ID-outliers-etl-data --attribute=objectId=$FILENAME`
    ...
    def _archive_file() -> Tuple[str, HTTPStatus]:
        ...
        if not filename.startswith("staging"):
            archive_etl_file(filename, OUTLIERS_VIEWS_OUTPUT_DIRECTORY_URI)
