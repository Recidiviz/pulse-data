...
def _parse_date(filename: str) -> datetime.date:
    ...
    if DATE_PARSE_ANCHOR_FILENAME in filename_date:
        ...
        start = filename_date.index(DATE_PARSE_ANCHOR_FILENAME) + len(
            DATE_PARSE_ANCHOR_FILENAME
        )
    ...
    try:
        return datetime.datetime.strptime(
            filename.split("/")[-1], "_wp-content_uploads_%Y_%m_abbrerptcurrent.pdf"
        )
...
def _parse_table(_: str, filename: str, report_date: datetime.date) -> pd.DataFrame:
    ...
    for page_num in range(1, num_pages + 1):
        ...
        for column in df.columns[1:]:
            ...
            if " " in str(df[column].iloc[0]):
                ...
                df_temp = pd.DataFrame(df.pop(column).str.split(n=1, expand=True))
    ...
    for i, page in enumerate(pages):
        ...
        page = aggregate_ingest_utils.rename_columns_and_select(page, columns_to_schema)
...
def _get_column_names(report_date):
    ...
    columns_to_schema = OrderedDict(
        [
            ...
            ("Parole Violators with a New Charge", "parole_violators_with_new_charge"),
