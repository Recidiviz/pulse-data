...
def mv_from_db_to_gcs(region_name):
    ...
    for entry in response.entries:
        ...
        logging.info("The path to download from dropbox is %s", db_path)
        ...
        gcs_path = os.path.join(bucket, region_name)
        gcs_file = fs.GCSFile(gcs_path, mode='wb')
        gcs_file.write(resp.content)
        gcs_file.close()

        dbx.files_move(db_path, processed_db_folder_path,
