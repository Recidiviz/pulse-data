...
from recidiviz.ingest.direct.controllers.gcsfs_direct_ingest_utils import gcsfs_direct_ingest_directory_path_for_region
...
class SftpAuth:
    ...
    def __init__(self, hostname: str, username: Optional[str],
                 password: Optional[str], connection_options: Optional[CnOpts]):
    ...
    def set_up_connection_options(prefix: str, host: str) -> CnOpts:
        ...
        try:
            ...
            if hostkey is None:
                raise ValueError(f"Unable to find hostkey for secret key {prefix}_hostkey") from s
            ...
            if hostkeyEntry:
                ...
                raise ValueError(f"Unable to add hostkey to connection_options for secret key {prefix}_hostkey") from s
...
class DownloadFilesFromSftpController:
    ...
    def __init__(self,
        ...
        self.download_dir = GcsfsDirectoryPath.from_dir_and_subdir(dir_path=self.bucket, subdir=RAW_INGEST_DIRECTORY)
    ...
    def _fetch(self, connection: pysftp.Connection, file_path: str, file_timestamp: datetime.datetime) -> None:
        ...
        try:
            ...
            self.gcsfs.upload_from_contents_handle_stream(path=path,
            ...
            self.downloaded_items.append((self.delegate.post_process_downloads(path, self.gcsfs), file_timestamp))
        except IOError as e:
            logging.info("Could not download %s into %s: %s", normalized_sftp_path, self.download_dir, e.args)
    ...
    def get_paths_to_download(self) -> List[Tuple[str, datetime.datetime]]:
        ...
        with pysftp.Connection(host=self.auth.hostname, username=self.auth.username,
                               password=self.auth.password, cnopts=self.auth.connection_options) as connection:
            ...
            paths_post_timestamp = {sftp_attr.filename: datetime.datetime.fromtimestamp(sftp_attr.st_mtime)
                                    for sftp_attr in dirs_with_attributes
                                    if self._is_after_update_bound(sftp_attr)}
            paths_to_download = self.delegate.filter_paths(list(paths_post_timestamp.keys()))
            ...
            for path in paths_to_download:
                ...
                if connection.isdir(path):
                    def set_file(file_to_fetch: str, file_timestamp: datetime.datetime) -> None:
                        files_to_download_with_timestamps.append((file_to_fetch, file_timestamp))

                    connection.walktree(remotepath=path,
    ...
    def clean_up(self) -> None:
        ...
        try:
            ...
            logging.info("%s could not be cleaned up due to an error %s.", self.download_dir.uri(), e.args)

    def fetch_files(self,
                    files_to_download_with_timestamps: List[Tuple[str, datetime.datetime]]) -> None:
    ...
    def do_fetch(self) -> Tuple[List[Tuple[str, datetime.datetime]], List[str]]:
        ...
        logging.info("Downloading raw files from SFTP server [%s] to ingest bucket [%s] for project [%s]",
                     self.auth.hostname, self.bucket.uri(), self.project_id)

        files_to_download_with_timestamps = self.get_paths_to_download()
        logging.info("Found %s items to download from SFTP server [%s] to upload to ingest bucket [%s]",
                     len(files_to_download_with_timestamps), self.auth.hostname, self.bucket)

        self.fetch_files(files_to_download_with_timestamps)

        logging.info("Download complete, successfully downloaded %s files to ingest bucket [%s] "
                     "could not download %s files",
                     len(self.downloaded_items), self.download_dir.uri(), len(self.unable_to_download_items))
