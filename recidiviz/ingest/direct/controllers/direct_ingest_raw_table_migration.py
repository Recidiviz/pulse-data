...
class UpdateRawTableMigration(RawTableMigration):
    ...
    UPDATE_MIGRATION_TEMPLATE = """UPDATE `{raw_table}` SET {update_clause} WHERE {filter_clause}{update_datetime_clause};"""
    ...
    def __init__(
        ...
        self._update_clause = ", ".join(
            [f"{field} = '{value}'" for field, value in updates]
        )
...
class DeleteFromRawTableMigration(RawTableMigration):
    ...
    DELETE_FROM_MIGRATION_TEMPLATE = (
        """DELETE FROM `{raw_table}` WHERE {filter_clause}{update_datetime_clause};"""
    )
