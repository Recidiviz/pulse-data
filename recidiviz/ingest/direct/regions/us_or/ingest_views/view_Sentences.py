...
charge AS ( 
  SELECT DISTINCT
    ...
    CASE 
      WHEN LENGTH(ORS_NUMBER) > 3 THEN
        CONCAT(SUBSTR(ORS_NUMBER, 1, LENGTH(ORS_NUMBER) - 3), '.', SUBSTR(ORS_NUMBER, LENGTH(ORS_NUMBER) - 2))
      ELSE ORS_NUMBER 
    END AS ORS_NUMBER,
    ORS_PARAGRAPH,
    CRIME_ABBREVIATION,
...
final AS (
  SELECT 
    ...
    charge.COUNTY,
    NCIC_CODE,
    IF(charge.ORS_PARAGRAPH IS NULL, charge.ORS_NUMBER, charge.ORS_NUMBER || '('|| charge.ORS_PARAGRAPH || ')') AS ORS_STATUTE,
    charge.CRIME_CLASS, 
