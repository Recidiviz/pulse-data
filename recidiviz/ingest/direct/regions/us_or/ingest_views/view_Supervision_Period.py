...
high_level_transfers AS (
  ...
  SELECT
    ...
    PROXY_RISK_LEVEL,
    CASE 
      WHEN PSC_RISK_LEVEL = 'LOW' THEN 'Low'
      WHEN PSC_RISK_LEVEL = 'MED' THEN 'Medium'
      WHEN PSC_RISK_LEVEL = 'HI' THEN 'High'
    ELSE PSC_RISK_LEVEL END AS PSC_RISK_LEVEL, 
...
supervision_level AS (
   SELECT 
    ...
    EFFECTIVE_DATE,
    ASSESSMENT_TOOL,
    IFNULL(PROXY_RISK_LEVEL, PSC_RISK_LEVEL) AS RISK_LEVEL,
  ...
  SELECT 
    ...
    EFFECTIVE_DATE, 
    RISK_LEVEL,
    ROW_NUMBER() OVER (PARTITION BY RECORD_KEY, EFFECTIVE_DATE ORDER BY CASE WHEN RISK_LEVEL = 'High' THEN 1 WHEN RISK_LEVEL = 'Medium' THEN 2 WHEN RISK_LEVEL = 'Low' THEN 3 ELSE 4 END, ASSESSMENT_TOOL DESC) AS weighted_suplevel
