...
split_periods_with_multiple_caseload_changes AS (
  SELECT DISTINCT
    ...
    COMMUNITY_SUPER_LVL,
    CASE 
      WHEN PERIOD_ID = LAST_PERIOD AND LAG(ADMISSION_REASON) OVER(PARTITION BY RECORD_KEY ORDER BY PERIOD_ID, MOVE_IN_DATE) != 'ABSC' THEN 'CASELOAD_CHANGE'
      WHEN PERIOD_ID = LAST_PERIOD AND LAG(ADMISSION_REASON) OVER(PARTITION BY RECORD_KEY ORDER BY PERIOD_ID, MOVE_IN_DATE) = 'ABSC' THEN 'ABSC'
    ELSE ADMISSION_REASON END AS ADMISSION_REASON,
    CASE 
      WHEN PERIOD_ID = NEXT_PERIOD AND ADMISSION_REASON != 'ABSC' THEN 'CASELOAD_CHANGE'
      WHEN PERIOD_ID = NEXT_PERIOD AND ADMISSION_REASON = 'ABSC' THEN 'ABSC'
    ELSE RELEASE_REASON END AS RELEASE_REASON,
    RESPONSIBLE_DIVISION, 
    CASELOAD
