...
WITH agents AS (
  ...
  proto_supervisor_periods AS (
    SELECT
      ...
      BadgeNumber,
      ParoleUnit,
      LAG(ParoleUnit) OVER (PARTITION BY possible_BadgeNumber, possible_ParoleUnit ORDER BY start_date) IS NOT NULL AS continuing_in_same_unit,
      LEAD(ParoleUnit) OVER (PARTITION BY possible_BadgeNumber, possible_ParoleUnit ORDER BY start_date) IS NOT NULL AS continuing_onto_same_unit
    FROM
      filled_all_dates 
    WHERE True
    ...
    QUALIFY (
      ParoleUnit IS NOT NULL AND (
        ...
        NOT continuing_in_same_unit OR
  ...
  proto_supervisor_periods_2 AS (
    SELECT
      start_date,
      LEAD(end_date) OVER (PARTITION BY BadgeNumber, ParoleUnit ORDER BY start_date) AS end_date,
      BadgeNumber,
      ParoleUnit,
      continuing_in_same_unit
    FROM
  ...
  collapsed_supervisor_periods AS (
    ...
    WHERE NOT continuing_in_same_unit
  ),
