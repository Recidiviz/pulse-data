...
INCARCERATION_PERIOD_ADMISSION_REASON_TO_STR_MAPPINGS: Dict[StateIncarcerationPeriodAdmissionReason, List[str]] = {
...
STR_TO_INCARCERATION_PERIOD_ADMISSION_REASON_MAPPINGS: Dict[str, StateIncarcerationPeriodAdmissionReason] = \
    invert_enum_to_str_mappings(INCARCERATION_PERIOD_ADMISSION_REASON_TO_STR_MAPPINGS)


def rank_incarceration_period_admission_reason_status_str(status_str: str) -> Optional[int]:
    ...
    """
    if (status_str in PAROLE_REVOKED_REENTRY_STATUS_CODES
    ...
    if (status_str in NEW_COURT_COMMITTMENT_STATUS_CODES
...
def incarceration_period_admission_reason_mapper(status_list_str: str) -> StateIncarcerationPeriodAdmissionReason:
    ...
    for status_str in statuses_at_rank:
        if status_str not in STR_TO_INCARCERATION_PERIOD_ADMISSION_REASON_MAPPINGS:
            raise ValueError(f'No mapping for incarceration admission status {status_str}')
        potential_admission_reasons.add(STR_TO_INCARCERATION_PERIOD_ADMISSION_REASON_MAPPINGS[status_str])
