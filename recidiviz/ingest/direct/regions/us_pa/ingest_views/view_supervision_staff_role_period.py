...
-- pull earliest dates from relagenthistory, assume all these people are still employed
    ...
    WHERE NOT (UPPER(agent_roster.FirstName) LIKE '%VACANT%' AND UPPER(agent_roster.LastName) LIKE '%POSITION%')
    GROUP BY 1
),
...
FROM (
    ...
    AND PRL_AGNT_JOB_CLASSIFCTN IN ('Prl Agt 1', 'Prl Agt 2','Prl Supv','Prl Mgr 1')
    ) cntc_with_row_num
WHERE rn = 1
),
complete_base AS (
