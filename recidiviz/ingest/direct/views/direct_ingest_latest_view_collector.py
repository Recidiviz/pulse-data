...
class DirectIngestRawDataTableLatestViewBuilder(BigQueryViewBuilder[DirectIngestRawDataTableLatestView]):
    ...
    def _build(self, *, dataset_overrides: Optional[Dict[str, str]] = None) -> DirectIngestRawDataTableLatestView:
        return DirectIngestRawDataTableLatestView(project_id=self.project_id,
...
class DirectIngestRawDataTableLatestViewCollector(BigQueryViewCollector[DirectIngestRawDataTableLatestViewBuilder]):
    def collect_view_builders(self) -> List[DirectIngestRawDataTableLatestViewBuilder]:
        ...
        for region_code in get_existing_region_dir_names():
            ...
            builder_list.extend([
                DirectIngestRawDataTableLatestViewBuilder(region_code=region_code,
                ...
                for config in raw_file_configs.values() if config.primary_key_cols])
        return builder_list
