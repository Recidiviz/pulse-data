...
def find_state_codes_in_str(s: str) -> "set[StateCode]":
    ...
    # Use _FakeStateCode directly because StateCode is resolved via __getattr__
    # at the module level and isn't available as a global inside this module.
    # _FakeStateCode is a superset of _RealStateCode (includes test codes).
    found: set[_FakeStateCode] = set()
    for match in _STATE_CODE_PATTERN.finditer(s.lower()):
        try:
            found.add(_FakeStateCode(match.group("state").upper()))
        except ValueError:
            pass
