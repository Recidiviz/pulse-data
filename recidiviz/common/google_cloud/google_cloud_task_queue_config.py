...
DIRECT_INGEST_QUEUE_BASE_CONFIG = tasks_v2.Queue(
...
BIGQUERY_QUEUE_CONFIG = tasks_v2.Queue(
...
JOB_MONITOR_QUEUE_CONFIG = tasks_v2.Queue(
...
BASE_SCRAPER_QUEUE_CONFIG = tasks_v2.Queue(
    rate_limits=tasks_v2.RateLimits(
...
SCRAPER_PHASE_QUEUE_CONFIG = tasks_v2.Queue(
    rate_limits=tasks_v2.RateLimits(
        max_dispatches_per_second=100,
...
def _queue_config_with_name(
        ...
        base_config: tasks_v2.Queue,
        queue_name: str) -> tasks_v2.Queue:
    queue = tasks_v2.Queue(base_config)
    queue.name=client_wrapper.format_queue_path(queue_name)
    return queue


def _apply_overrides(message: proto.Message, overrides: dict) -> None:
    for key, value in overrides.items():
        # If this is a dictionary that indicates that the field is a nested message. We don't want to override the
        # entire message so we recurse to only override the fields specified in the dictionary.
        if isinstance(value, dict):
            _apply_overrides(getattr(message, key), value)
        else:
            setattr(message, key, value)


def _build_cloud_task_queue_configs(
        client_wrapper: GoogleCloudTasksClientWrapper) -> List[tasks_v2.Queue]:
    ...
    scraper_phase_queue = _queue_config_with_name(client_wrapper,
                                    SCRAPER_PHASE_QUEUE_CONFIG,
                                    SCRAPER_PHASE_QUEUE_V2)
    scraper_phase_queue.rate_limits=tasks_v2.RateLimits(max_dispatches_per_second=1)
    queues.append(scraper_phase_queue)
    ...
    for vendor in vendors.get_vendors():
        ...
        queue = _queue_config_with_name(client_wrapper,
                                    ...
                                    vendor_queue_name)
        _apply_overrides(queue, overrides=queue_params)
        queues.append(queue)
    ...
    for region in regions.get_supported_regions():
        ...
        if region.queue:
            _apply_overrides(queue, overrides=region.queue)
        queues.append(queue)
