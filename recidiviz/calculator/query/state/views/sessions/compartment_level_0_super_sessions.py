...
# =============================================================================
"""View that aggregates across sessions with matching adjacent compartment_level_0 values in compartment_sessions"""
...
COMPARTMENT_LEVEL_0_SUPER_SESSIONS_VIEW_NAME = "compartment_level_0_super_sessions"

COMPARTMENT_LEVEL_0_SUPER_SESSIONS_VIEW_DESCRIPTION = """
## Overview

This view is unique on `person_id` and `compartment_level_0_super_session_id` and aggregates across adjacent `compartment_level_0` values. `compartment_level_0` groups the `compartment_level_1` values `INCARCERATION` and `INCARCERATION_OUT_OF_STATE` into `INCARCERATION`, and `SUPERVISION` and `SUPERVISION_OUT_OF_STATE` into `SUPERVISION`. This view allows us to easily calculate the continuous time that an individual spends under incarceration or supervision regardless of whether they are in-state or out-of-state. 

## Field Definitions
|	Field	|	Description	|
|	--------------------	|	--------------------	|
|	person_id	|	Unique person identifier	|
|	compartment_level_0_super_session_id	|	Session id for the super session. Aggregates across compartment_level_0 values	|
...
|	end_date_exclusive	|	Super-session exclusive end date|
|	state_code	|	State	|
...
COMPARTMENT_LEVEL_0_SUPER_SESSIONS_QUERY_TEMPLATE = """
    ...
    SELECT
        ...
        SUM(CASE WHEN change_compartment THEN 1 ELSE 0 END) 
            OVER(PARTITION BY person_id ORDER BY start_date) + 1 AS compartment_level_0_super_session_id
    FROM 
        (
        /*
        Identifies compartments where the compartment_level_0 value changes from the previous compartment_level_0
        value.
        */
        SELECT
            *,
            COALESCE(compartment_level_0 != LAG(compartment_level_0) OVER(PARTITION BY person_id ORDER BY start_date),FALSE) change_compartment,
        FROM 
            (
            SELECT
                *,
    ...
    */
    SELECT 
        s.person_id,
        s.compartment_level_0_super_session_id,
        s.compartment_level_0,
        s.start_date,
        ...
        last.end_reason,
        first.compartment_level_1 AS compartment_level_1_start,
        last.compartment_level_1 AS compartment_level_1_end,
...
COMPARTMENT_LEVEL_0_SUPER_SESSIONS_VIEW_BUILDER = SimpleBigQueryViewBuilder(
    dataset_id=SESSIONS_DATASET,
    view_id=COMPARTMENT_LEVEL_0_SUPER_SESSIONS_VIEW_NAME,
    view_query_template=COMPARTMENT_LEVEL_0_SUPER_SESSIONS_QUERY_TEMPLATE,
    description=COMPARTMENT_LEVEL_0_SUPER_SESSIONS_VIEW_DESCRIPTION,
...
if __name__ == "__main__":
    with local_project_id_override(GCP_PROJECT_STAGING):
        COMPARTMENT_LEVEL_0_SUPER_SESSIONS_VIEW_BUILDER.build_and_print()
