...
WITH
  ...
  LEFT JOIN
    `{project_id}.{us_ix_raw_data_up_to_date_dataset}.ref_Employee_latest` e
  ON
    psi.AssignedToUserId = e.EmployeeId
  LEFT JOIN
  ...
  LEFT JOIN
    `{project_id}.{us_ix_raw_data_up_to_date_dataset}.ind_OffenderNoteInfo_latest`
  USING
    (OffenderNoteInfoId)
  LEFT JOIN
    `{project_id}.{us_ix_raw_data_up_to_date_dataset}.ind_OffenderNoteInfo_ContactMode_latest`
  USING
    (OffenderNoteInfoId)
  LEFT JOIN
    `{project_id}.{us_ix_raw_data_up_to_date_dataset}.ind_ContactMode_latest`
  USING
    (ContactModeId)
  LEFT JOIN
    `{project_id}.{us_ix_raw_data_up_to_date_dataset}.com_PSIReport_latest` psi
  ...
  WHERE
    NoteDate >= OrderDate
    AND NoteDate < CompletedDate
    AND ContactModeDesc IN ("PSI File Review Assigned",
      "PSI Assigned Full",
  ...
  FROM (
    ...
    FROM
      `{project_id}.{normalized_state_dataset}.state_assessment` lsir )
  WHERE
    recency_rank = 1 ),
  ...
  END
    AS lsir_level
  FROM
    case_info_with_report_type cirt
  LEFT JOIN
    most_recent_score_cte mrs
  ON
    mrs.person_id = cirt.person_id)
SELECT
  state_code,
  external_id,
  client_id,
