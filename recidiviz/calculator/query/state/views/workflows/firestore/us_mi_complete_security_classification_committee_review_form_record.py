...
US_MI_COMPLETE_SECURITY_CLASSIFICATION_COMMITTEE_REVIEW_FORM_RECORD_QUERY_TEMPLATE = f"""
...
GROUP BY person_id
 ...
 eligible_and_almost_eligible AS (
    ...
    FROM (
        ...
        FROM 
            `{{project_id}}.{{task_eligibility_dataset}}.complete_security_classification_committee_review_form_materialized` c
        ) AS tes
...
/* Queries the reasons for eligibility from the currently eligible or upcoming eligible span */
    SELECT 
        * EXCEPT(array_reasons),
        CAST(
                ARRAY(
                SELECT JSON_VALUE(x.reason.latest_scc_review_date)
                FROM UNNEST(array_reasons) AS x
                WHERE STRING(x.criteria_name) = 'US_MI_PAST_SECURITY_CLASSIFICATION_COMMITTEE_REVIEW_DATE'
                )[OFFSET(0)]
...
US_MI_COMPLETE_SECURITY_CLASSIFICATION_COMMITTEE_REVIEW_FORM_RECORD_VIEW_BUILDER = SimpleBigQueryViewBuilder(
    dataset_id=dataset_config.WORKFLOWS_VIEWS_DATASET,
    view_id=US_MI_COMPLETE_SECURITY_CLASSIFICATION_COMMITTEE_REVIEW_FORM_RECORD_VIEW_NAME,
    view_query_template=US_MI_COMPLETE_SECURITY_CLASSIFICATION_COMMITTEE_REVIEW_FORM_RECORD_QUERY_TEMPLATE,
