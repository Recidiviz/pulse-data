...
from recidiviz.calculator.query.state.views.workflows.firestore.utils import (
    get_workflows_salt_value,
)
...
PSEUDONYMIZED_ID = """
    ...
    SUBSTRING(
        ...
        REPLACE(
            REPLACE(
                TO_BASE64(SHA256(state_code || person_external_id || '{secret_salt}')), 
                '+', 
                '-'
            ), 
            '/', 
...
RESIDENT_RECORD_QUERY_TEMPLATE = f"""
    WITH 
        {US_ME_INCARCERATION_RESIDENTS_QUERY_TEMPLATE}
    SELECT *, {PSEUDONYMIZED_ID} AS pseudonymized_id FROM me_residents
"""
...
RESIDENT_RECORD_VIEW_BUILDER = SimpleBigQueryViewBuilder(
    ...
    secret_salt=get_workflows_salt_value(),
