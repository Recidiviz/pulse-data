...
COMPLETE_TRANSFER_TO_LIMITED_SUPERVISION_FORM_US_ID_RECORD_VIEW_NAME = (
    "us_id_complete_transfer_to_limited_supervision_form_record"
)

COMPLETE_TRANSFER_TO_LIMITED_SUPERVISION_FORM_US_ID_RECORD_DESCRIPTION = """
    ...
    """
COMPLETE_TRANSFER_TO_LIMITED_SUPERVISION_FORM_US_ID_RECORD_QUERY_TEMPLATE = f"""
 /*{{description}}*/
   ...
   notes AS(
    ...
    form AS (
      SELECT
          ...
          ARRAY_AGG(charge.description IGNORE NULLS) AS form_information_charge_descriptions,
      ...
      LEFT JOIN sentence_charge_description charge
        ...
        --select charges that were imposed before the relevant supervision session and that 
        --expire after the eligible period has started
        AND ses.start_date >= charge.date_imposed 
        AND tes.start_date <= COALESCE(charge.projected_completion_date_max, "9999-12-31")
      ...
      LEFT JOIN latest_assessment_score score
        ...
        --join all latest? or only latest when assessment_score_date <= ses.start_date
      ...
      LEFT JOIN latest_drug_screen ds
        ...
        --join all latest? or only latest when drug_screen_date <= ses.start_date
      ...
      LEFT JOIN latest_treatment_discharge tx
        ...
        AND tx.discharge_date BETWEEN ses.start_date AND (COALESCE(ses.end_date, "9999-12-31"))
      LEFT JOIN notes n
...
COMPLETE_TRANSFER_TO_LIMITED_SUPERVISION_FORM_US_ID_RECORD_VIEW_BUILDER = SimpleBigQueryViewBuilder(
    dataset_id=dataset_config.WORKFLOWS_VIEWS_DATASET,
    view_id=COMPLETE_TRANSFER_TO_LIMITED_SUPERVISION_FORM_US_ID_RECORD_VIEW_NAME,
    view_query_template=COMPLETE_TRANSFER_TO_LIMITED_SUPERVISION_FORM_US_ID_RECORD_QUERY_TEMPLATE,
    description=COMPLETE_TRANSFER_TO_LIMITED_SUPERVISION_FORM_US_ID_RECORD_DESCRIPTION,
...
if __name__ == "__main__":
    with local_project_id_override(GCP_PROJECT_STAGING):
        COMPLETE_TRANSFER_TO_LIMITED_SUPERVISION_FORM_US_ID_RECORD_VIEW_BUILDER.build_and_print()
