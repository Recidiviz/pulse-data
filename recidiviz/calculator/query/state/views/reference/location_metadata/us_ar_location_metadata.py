...
SELECT
    ...
    TO_JSON(
        STRUCT(
            ...
            op.ORGANIZATIONTYPE AS {LocationMetadataKey.LOCATION_SUBTYPE.value},
...
/*
In general, if a location's type doesn't fit into our schema, we set location_type to null 
and store ORGANIZATIONTYPE in the subtype. However, the vast majority of the data in the 
ORGANIZATIONPROF table is for external employers/businesses/other outside entities (covered
by type codes starting with 'M') that come up in various tables but exist fully outside the 
scope of ADC/ACC, and don't need location metadata. Therefore, these are filtered out 
entirely, along with 'Z9' ('Unknown') locations and 'JS' locations (this is an unrecognized
code value, and seems to be used for flagging hot check alerts, not actual locations).
*/

WHERE op.ORGANIZATIONTYPE NOT LIKE 'M%' AND
    op.ORGANIZATIONTYPE NOT IN ('Z9','JS')
"""
