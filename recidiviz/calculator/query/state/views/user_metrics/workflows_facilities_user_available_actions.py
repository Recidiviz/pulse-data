...
WORKFLOWS_FACILITIES_USER_AVAILABLE_ACTIONS_VIEW_NAME = (
    "workflows_facilities_user_available_actions"
)

WORKFLOWS_FACILITIES_USER_AVAILABLE_ACTIONS_QUERY_TEMPLATE = """
SELECT
    metrics.state_code,
    metrics.facility_counselor_id,
    metrics.facility_counselor_name,
...
FROM
    `{project_id}.user_metrics.workflows__incarceration_facility_counselor_aggregated_metrics_materialized` metrics
INNER JOIN
...
ON
    ...
    AND users.staff_id = metrics.facility_counselor_id
...
WORKFLOWS_FACILITIES_USER_AVAILABLE_ACTIONS_VIEW_BUILDER = SimpleBigQueryViewBuilder(
    dataset_id=dataset_config.USER_METRICS_DATASET_ID,
    view_id=WORKFLOWS_FACILITIES_USER_AVAILABLE_ACTIONS_VIEW_NAME,
    view_query_template=WORKFLOWS_FACILITIES_USER_AVAILABLE_ACTIONS_QUERY_TEMPLATE,
    description=__doc__,
...
if __name__ == "__main__":
    with local_project_id_override(GCP_PROJECT_PRODUCTION):
        WORKFLOWS_FACILITIES_USER_AVAILABLE_ACTIONS_VIEW_BUILDER.build_and_print()
