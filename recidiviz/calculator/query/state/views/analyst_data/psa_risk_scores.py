...
# =============================================================================
"""
Creates a view to gather data needed to calculate PSA risk scores for all supervised clients.

'The Public Safety Assessment (PSA) is an actuarial assessment that predicts failure to appear in court pretrial,
new criminal arrest while on pretrial release, and new violent criminal arrest while on pretrial release. Use of the
PSA, in combination with other pretrial improvements, is associated with improved outcomes. These include higher
rates of pretrial release and less use of financial conditions of release. These outcomes do not negatively impact
crime or court appearance rates.'
...
"The Public Safety Assessment (PSA) is an actuarial assessment that predicts failure to appear in court pretrial, 
new criminal arrest while on pretrial release, and new violent criminal arrest while on pretrial release. Use of 
the PSA, in combination with other pretrial improvements, is associated with improved outcomes. These include 
higher rates of pretrial release and less use of financial conditions of release. These outcomes do not 
negatively impact crime or court appearance rates." 
...
, supervision_starts_w_reincarcerations AS (
    SELECT
        ...
        SUM(IFNULL(b.reincarcerated, 0)) > 0 AS reincarcerated_within_one_year_of_supervision_start
    FROM 
...
Strategy:
- For each supervision session
1. Identify whether the most recent sentence included a violent charge. 
    a. We use the "*_uniform" fields in `sentences_preprocessed` to keep the score is state agnostic (different states
        can have different criteria for crime categories)
2. To calculate priors, identify all sentences that were imposed on or prior to the start date of the session
...
, charge_cte AS (
    ...
    FROM
        supervision_starts a
    INNER JOIN
        `{project_id}.{sessions_dataset}.sentences_preprocessed_materialized` b
    ON
        a.state_code = b.state_code
        AND a.person_id = b.person_id
        ...
        AND IFNULL(b.date_imposed, b.effective_date) BETWEEN DATE_SUB(a.start_date, INTERVAL 10 YEAR) AND a.start_date
)
...
, current_violent_cte AS (
    SELECT
        ...
        person_id,
        session_id,
        COUNTIF(is_violent_uniform) AS current_violent,
        ...
        COUNTIF(is_missing_violent AND NOT is_misdemeanor) as current_missing_violent,
    FROM
        charge_cte
    WHERE
        is_current_charge       
...
, prior_charges_cte AS (
    SELECT
        ...
        person_id,
        session_id,
        COUNTIF(is_misdemeanor) AS prior_misdemeanors,
        COUNTIF(is_felony) AS prior_felonies,
        COUNTIF(is_missing_classification) AS prior_missing_classification,
        COUNTIF(is_violent_uniform) AS prior_violent,
        COUNTIF(is_missing_violent AND NOT is_misdemeanor) AS prior_missing_violent,
        COUNTIF(is_incarceration_sentence) AS prior_sentences_to_incarceration, 
    FROM
        charge_cte
    GROUP BY
        1, 2, 3   
...
, prior_absconsions_cte AS (
    SELECT
        ...
        IFNULL(COUNTIF(DATE_DIFF(a.start_date, b.start_date, DAY) / 365.25 BETWEEN 2 and 10), 0) 
            AS prior_absconsions_older_than_two_years,
    FROM
        supervision_starts a
...
, psa_item_scores_cte AS (
    SELECT 
        *,
        ...
        IF(prior_felonies > 0 OR prior_misdemeanors > 0 OR prior_missing_classification > 0, 1, 0)  
        ...
        IF((prior_felonies + prior_misdemeanors + prior_missing_classification) > 1, 1, 0)
        ...
        IF((prior_felonies + prior_misdemeanors + prior_missing_classification) > 0, 1, 0)
    ...
    # client must have a prior charge (since current charge is a subset of prior charges in the supervision setting)
    INNER JOIN
        prior_charges_cte
    USING
        (state_code, person_id, session_id)
