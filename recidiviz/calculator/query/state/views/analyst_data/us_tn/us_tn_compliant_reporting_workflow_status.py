...
US_TN_COMPLIANT_REPORTING_WORKFLOW_STATUS_QUERY_TEMPLATE = """
    ...
    eligible_clients AS (
      SELECT
            cl.person_id,
            ARRAY_AGG(cr.compliant_reporting_eligible ORDER BY cr.export_date DESC)[OFFSET(0)] AS compliant_reporting_eligible,
            ARRAY_AGG(cl.district ORDER BY cl.export_date DESC)[OFFSET(0)] AS district,
            ARRAY_AGG(cl.officer_id ORDER BY cl.export_date DESC)[OFFSET(0)] AS officer_id,
            ...
            ARRAY_AGG(cr.remaining_criteria_needed ORDER BY cr.export_date DESC)[OFFSET(0)] AS remaining_criteria_needed_latest,
            ...
            MIN(cr.export_date) AS eligible_start,
            MAX(IFNULL(DATE_SUB(cr.next_export, INTERVAL 1 DAY), CURRENT_DATE("US/Eastern"))) AS eligible_end,
        ...
        USING (person_id, export_date, state_code)
        GROUP BY 1
    )
    , surfaced AS (
        SELECT
