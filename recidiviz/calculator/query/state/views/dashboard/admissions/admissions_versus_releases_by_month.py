...
ADMISSIONS_VERSUS_RELEASES_BY_MONTH_QUERY_TEMPLATE = """
    ...
    FULL OUTER JOIN (
      ...
      FROM `{project_id}.{materialized_metrics_dataset}.most_recent_incarceration_release_metrics_included_in_state_population_materialized`,
      {district_dimension}
    ...
    FULL OUTER JOIN (
      ...
      {district_dimension}
      WHERE year >= EXTRACT(YEAR FROM DATE_SUB(CURRENT_DATE('US/Eastern'), INTERVAL 4 YEAR))
