...
SELECT
  ...
  IF(input.county_code IS NULL,
     CAST(ARRAY_LENGTH(input.collapsed_dimension_values) AS FLOAT64) / state_info.num_counties,
     NULL) as percentage_covered_county,
  IF(input.county_code IS NULL,
     CAST(input.resident_population AS FLOAT64) / state_info.total_population,
    ...
    unfiltered_state_jail_pop = SimpleBigQueryViewBuilder(
        ...
        FROM (
          SELECT
            ...
            1 as ordinal
          FROM `{project_id}.{input_dataset}.{input_table}`
          WHERE '{state_dimension_identifier}' = ARRAY_TO_STRING(ARRAY(SELECT dimension from UNNEST(dimensions)), '|')
        ) as foo
        """,
    ...
    def __init__(self) -> None:
        ...
        county_coverage_view = SimpleBigQueryViewBuilder(
            ...
            SELECT
                ...
                month,
                date_reported,
                CAST(NULL as STRING) as measurement_type,
                percentage_covered_county as value,
                CAST(NULL as INT64) as compared_to_year,
                CAST(NULL as INT64) as compared_to_month,
                CAST(NULL as NUMERIC) as value_change,
                CAST(NULL as NUMERIC) as percentage_change,
                CAST(NULL as FLOAT64) as percentage_covered_county,
                CAST(NULL as FLOAT64) as percentage_covered_population,
                date_published
            FROM `{project_id}.{input_dataset}.{input_table}`
