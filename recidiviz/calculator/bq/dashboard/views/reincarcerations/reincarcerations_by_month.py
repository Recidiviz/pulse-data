...
REINCARCERATIONS_BY_MONTH_QUERY = \
    ...
    SELECT EXTRACT(YEAR FROM start_date) as year, EXTRACT(MONTH FROM start_date) as month, returns
    ...
    AND EXTRACT(YEAR FROM start_date) > EXTRACT(YEAR FROM DATE_ADD(CURRENT_DATE(), INTERVAL -2 YEAR))
