...
from recidiviz.calculator.pipeline.utils.revocation_utils import get_revocation_details, \
    has_revocation_admission_on_date
from recidiviz.calculator.pipeline.utils.state_utils.state_calculation_config_manager import \
    ...
    get_state_specific_case_compliance_manager, include_decisions_on_follow_up_responses_for_most_severe_response, \
...
from recidiviz.calculator.pipeline.utils.violation_response_utils import prepare_violation_responses_for_calculations, \
...
from recidiviz.common.constants.state.state_supervision_violation_response \
    import StateSupervisionViolationResponseDecision, StateSupervisionViolationResponseType
...
def find_time_buckets_for_supervision_period(
    ...
    while event_date < end_date:
        if on_supervision_on_date(
            ...
            violation_history = get_violation_and_response_history(event_date,
                                                                   violation_responses)
            ...
            projected_end_date = _get_projected_end_date(incarceration_sentences=incarceration_sentences,
...
def find_supervision_termination_bucket(
    ...
    if supervision_period.start_date is not None and supervision_period.termination_date is not None:
        ...
        violation_history = get_violation_and_response_history(termination_date, violation_responses)
...
def get_violation_and_response_history(
    ...
    violation_window_lower_bound_inclusive = (end_date - relativedelta(months=VIOLATION_HISTORY_WINDOW_MONTHS))
...
def _get_projected_end_date(supervision_sentences: List[StateSupervisionSentence],
                            incarceration_sentences: List[StateIncarcerationSentence],
                            period: PeriodType) -> \
        Optional[date]:
...
def _sorted_violation_responses_in_window(violation_responses: List[StateSupervisionViolationResponse],
    ...
    state_filtered_responses = \
        state_specific_filter_of_violation_responses(filtered_responses, include_follow_up_responses)

    responses_in_window = [
        response for response in state_filtered_responses
        if response.response_date is not None
        and response.response_date < upper_bound_exclusive
        # Only limit with a lower bound if one is set
        and (lower_bound_inclusive is None
...
def find_revocation_return_buckets(supervision_sentences: List[StateSupervisionSentence],
    ...
    for index, incarceration_period in enumerate(incarceration_period_index.incarceration_periods):
        ...
        if responses_in_window:
            ...
            violation_history = get_violation_and_response_history(last_response.response_date, violation_responses)
        else:
        ...
        if include_decisions_on_follow_up_responses_for_most_severe_response(incarceration_period.state_code):
        ...
        if revoked_supervision_periods:
            ...
            for supervision_period in revoked_supervision_periods:
                revocation_details = get_revocation_details(
                ...
                projected_end_date = \
                    _get_projected_end_date(period=supervision_period,
                                            supervision_sentences=supervision_sentences,
                                            incarceration_sentences=incarceration_sentences)
            ...
            revocation_details = get_revocation_details(incarceration_period, None, violation_responses, None)
            ...
            projected_end_date = _get_projected_end_date(period=incarceration_period,
                                                         supervision_sentences=supervision_sentences,
                                                         incarceration_sentences=incarceration_sentences)
...
BUCKET_TYPES_FOR_METRIC: Dict[SupervisionMetricType, List[Type[SupervisionTimeBucket]]] = {
    ...
    SupervisionMetricType.SUPERVISION_DOWNGRADE: [NonRevocationReturnSupervisionTimeBucket]
}
...
def _get_supervision_downgrade_details_if_downgrade_occurred(supervision_period_index: SupervisionPeriodIndex,
    ...
    most_recent_previous_supervision_period: Optional[StateSupervisionPeriod] = \
        supervision_period_index.get_most_recent_previous_supervision_period(current_supervision_period)

    if current_supervision_period.supervision_level and most_recent_previous_supervision_period \
        ...
        if current_supervision_period.start_date != most_recent_previous_supervision_period.termination_date:
            return False, None

        return _supervision_level_downgrade_occurred(most_recent_previous_supervision_period.supervision_level,
                                                     current_supervision_period.supervision_level)

    return False, None


def _supervision_level_downgrade_occurred(previous_supervision_level: StateSupervisionLevel,
    ...
    if previous_supervision_level in not_applicable_supervision_levels or \
            current_supervision_level in not_applicable_supervision_levels:
        return False, None

    if previous_supervision_level not in supervision_level_to_number.keys():
        raise ValueError(f"Previous supervision level ({previous_supervision_level}) was not listed as a supervision "
    ...
    if current_supervision_level not in supervision_level_to_number.keys():
        raise ValueError(f"Current supervision level ({current_supervision_level}) was not listed as a supervision "
                         f"level that is out of scope for downgrades and was not found in the list of levels that can "
                         f"be compared for downgrades. Please add it to one of the two.")

    if supervision_level_to_number[previous_supervision_level] > supervision_level_to_number[current_supervision_level]:
