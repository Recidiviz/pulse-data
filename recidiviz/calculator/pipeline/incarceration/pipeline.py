...
class ClassifyIncarcerationEvents(beam.DoFn):
    ...
    def process(self, element, *args, **kwargs):
        ...
        if not incarceration_events:
            logging.info(
                "No valid incarceration events for person with"
...
class GetIncarcerationMetrics(beam.PTransform):
    """Transforms a StatePerson and IncarcerationEvents into
    ...
    def expand(self, input_or_inputs):
        # Calculate incarceration metric combinations from a StatePerson and
        # their IncarcerationEvents
        ...
        admission_metrics = (admissions_with_sums
                             | 'Produce admission count metrics' >>
                             beam.ParDo(
...
@with_input_types(beam.typehints.Tuple[entities.StatePerson,
                                       Dict[int, List[IncarcerationEvent]]])
...
class CalculateIncarcerationMetricCombinations(beam.DoFn):
    ...
    def process(self, element, *args, **kwargs):
        ...
        IncarcerationEvents for mapping all incarceration combinations.
        ...
        metric_combinations = \
            calculator.map_incarceration_combinations(person,
                                                      incarceration_events,
                                                      kwargs)
    ...
    def process(self, element, *args, **kwargs):
        ...
        Args:
            element: A tuple containing string representation of the metric_key
                for a given incarceration metric, and a dictionary containing
                the values for the given metric.
            **kwargs: This should be a dictionary with values for the
                following keys:
        ...
        if metric_type == MetricType.ADMISSION.value:
            ...
            incarceration_metric = \
                IncarcerationReleaseMetric.build_from_metric_key_group(
                    dict_metric_key, pipeline_job_id)
        else:
...
def run(argv=None):
    ...
    with beam.Pipeline(argv=pipeline_args) as p:
        ...
        writable_metrics = (incarceration_metrics
