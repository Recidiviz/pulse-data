...
# =============================================================================
"""Contains the logic for a ViolationResponsePreProcessingManager that manages the
pre-processing of StateSupervisionViolationResponse entities in the calculation
pipelines. """
import datetime
...
class StateSpecificViolationResponsePreProcessingDelegate:
    """Interface for state-specific decisions involved in pre-processing violation
...
class ViolationResponsePreProcessingManager:
    ...
    def __init__(
        ...
        delegate: StateSpecificViolationResponsePreProcessingDelegate,
    ):
    ...
    def pre_processed_violation_responses_for_calculations(
        ...
        """Performs pre-processing on violation responses. Filters out draft
        ...
        list of sorted, pre-processed StateSupervisionViolationResponses."""

        # Make a deep copy of the original violation responses to preprocess
        responses_for_pre_processing = deepcopy(self._violation_responses)

        filtered_responses = self._drop_responses_from_calculations(
            responses_for_pre_processing
        )
