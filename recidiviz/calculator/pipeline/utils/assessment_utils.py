...
from recidiviz.common.constants.state.state_assessment import \
    StateAssessmentType, StateAssessmentLevel, StateAssessmentClass
from recidiviz.persistence.entity.state.entities import StateAssessment

_ASSESSMENT_TYPES_TO_INCLUDE_FOR_CLASS: Dict[StateAssessmentClass, Dict[str, List[StateAssessmentType]]] = {
...
def _assessment_types_of_class_for_state(assessment_class: StateAssessmentClass,
                                         state_code: str) -> Optional[List[StateAssessmentType]]:
    ...
    if types_for_class:
        ...
        if not assessment_types_to_include:
            raise ValueError(f"Unsupported state_code {state_code} in _ASSESSMENT_TYPES_TO_INCLUDE_FOR_CLASS.")
...
def find_most_recent_applicable_assessment_of_class_for_state(cutoff_date: date,
                                                              assessments: List[StateAssessment],
                                                              assessment_class: StateAssessmentClass,
                                                              state_code: str) -> Optional[StateAssessment]:
    ...
    if assessments:
        assessment_types_to_include = _assessment_types_of_class_for_state(assessment_class, state_code)
...
def most_recent_applicable_assessment_attributes_for_class(cutoff_date: date,
                                                           assessments: List[StateAssessment],
                                                           assessment_class: StateAssessmentClass,
                                                           state_code: str) -> \
