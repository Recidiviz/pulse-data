...
# TRANSITIONS TABLE
transitions_data = pd.concat([transitions_data, pd.read_csv('recidiviz/calculator/modeling/population_projection/state/OK/OK_data/Transitions Data-Table 1.csv')])
transitions_data.loc[transitions_data.compartment == 'prison_technical', 'outflow_to'] = 'release'
transitions_data.loc[transitions_data.compartment == 'probation ', 'compartment'] = 'probation'

# OUTFLOWS TABLE
outflows_data = pd.concat([outflows_data, pd.read_csv('recidiviz/calculator/modeling/population_projection/state/OK/OK_data/Outflows Data-Table 1.csv')])
...
# STORE DATA
upload_spark_model_inputs('recidiviz-staging', 'OK_probation', outflows_data, transitions_data,
                          pd.DataFrame())
