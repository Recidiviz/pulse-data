...
# TRANSITIONS TABLE
transitions_data = pd.concat([transitions_data, pd.read_csv('recidiviz/calculator/modeling/population_projection/state/GA/GA_data/Transitions Data-Table 1.csv')])
transitions_data.loc[transitions_data.compartment == 'prison_probation_revocations', 'outflow_to'] = 'release'
transitions_data = transitions_data.rename({'placeholder_axis': 'crime_type'}, axis=1)
# OUTFLOWS TABLE
outflows_data = pd.concat([outflows_data, pd.read_csv('recidiviz/calculator/modeling/population_projection/state/GA/GA_data/Outflows Data-Table 1.csv')])
outflows_data = outflows_data.rename({'placeholder_axis': 'crime_type'}, axis=1)
#TOTAL POPULATION TABLE
total_population_data = pd.concat([total_population_data, pd.read_csv('recidiviz/calculator/modeling/population_projection/state/GA/GA_data/Total Population Data-Table 1.csv')])
total_population_data = total_population_data.rename({'placeholder_axis': 'crime_type'}, axis=1)
# STORE DATA
# STORE DATA
upload_spark_model_inputs('recidiviz-staging', 'GA_prison', outflows_data, transitions_data,
