...
# TRANSITIONS TABLE
transitions_data = pd.concat([transitions_data, pd.read_csv('recidiviz/calculator/modeling/population_projection/state/VA/HB_2038/VAHB2038_transitions_data.csv')])
...
transitions_data.loc[transitions_data.outflow_to == 'prison_technical', 'total_population'] *= 2.7 / (2.7 + 7.8)
transitions_data.loc[transitions_data.outflow_to == 'prison_newcrime', 'total_population'] *= 7.8 / (2.7 + 7.8)

COMPLETION_DURATION = 21
for crime_type in ['prison_newcrime', 'prison_technical']:
    chopped_total = transitions_data.loc[(transitions_data.compartment == 'probation') &
                                         (transitions_data.outflow_to == crime_type) &
                                         (transitions_data.compartment_duration > COMPLETION_DURATION), 'total_population'].sum()
    remaining_total = transitions_data.loc[(transitions_data.compartment == 'probation') &
                                         (transitions_data.outflow_to == crime_type) &
                                         (transitions_data.compartment_duration <= COMPLETION_DURATION), 'total_population'].sum()
    transitions_data.loc[(transitions_data.compartment == 'probation') &
                         (transitions_data.outflow_to == crime_type) &
                         (transitions_data.compartment_duration <= COMPLETION_DURATION), 'total_population'] *= (remaining_total + chopped_total) / remaining_total
    transitions_data = transitions_data[~((transitions_data.compartment == 'probation') &
                                         (transitions_data.outflow_to == crime_type) &
...
outflows_data = pd.DataFrame({
    ...
    'crime_type': ['NA'] * 7 * 12,
    'total_population': [33897 / 12] * 12 + [28885 / 12] * 12 + [28465 / 12] * 12 + [28831 / 12] * 12 +
                        [20539 / 12] * 12 + [24884 / 12] * 12 + [25625 / 12] * 12,
...
total_population_data = pd.concat([total_population_data, pd.read_csv('recidiviz/calculator/modeling/population_projection/state/VA/HB_2038/VAHB2038_total_population.csv')])
total_population_data = total_population_data[(total_population_data.time_step < 0)
...
total_population_data = total_population_data[(total_population_data.compartment != 'probation') |
                                              (total_population_data.crime_type != 'newcrime')]
# move disaggregation axis to compartments
prison_populations = total_population_data.loc[total_population_data.compartment == 'prison']
total_population_data.loc[total_population_data.compartment == 'prison', 'compartment'] = \
