...
total_population = pd.DataFrame({
    ...
    'felony': [247770, 249173, 254106, 244122, 221243, 190686, 183623, 166745, 161120],
    'misdemeanor': [50147, 45820, 42858, 41559, 42288, 49049, 49423, 43018, 38193]
})

discharges = pd.DataFrame({
    'time_step': [i - reference_year for i in range(2011, 2020)],
    'felony': [134055, 128129, 134849, 134970, 119320, 103172, 100745, 102212, 93245],
    'misdemeanor': [32984, 33414, 29911, 28105, 41846, 32994, 33198, 34426, 29294]
})

admissions = pd.DataFrame({
    'time_step': [i - reference_year for i in range(2011, 2020)],
    'felony': [131626, 132387, 142904, 140890, 111689, 104045, 104146, 90836, 89724],
...
#TRANSITIONS TABLE
mean_revocation_fraction_of_discharges = {crime: mean(revocations[crime]) / mean(discharges[crime])
                                          for crime in crimes}

mean_completion_duration = {crime: (1 - mean_revocation_fraction_of_discharges[crime]) * mean(total_population[crime]) /
...
for crime in crimes:
    ...
    transitions_data = pd.concat([transitions_data, probation_transition_table, secondary_transition_table])
...
for crime in crimes:
    ...
    outflows_data = \
        pd.concat([outflows_data, admissions_outflows_data, discharges_outflows_data, revocations_outflows_data])

#TOTAL POPULATION DATA
total_population_data = pd.DataFrame()
for crime in ['felony', 'misdemeanor']:
    crime_population_data = total_population[['time_step', crime]]
    crime_population_data = crime_population_data.rename({crime: 'total_population'}, axis=1)
