...
# TRANSITIONS TABLE
transitions_data = pd.concat([transitions_data, pd.read_csv('recidiviz/calculator/modeling/population_projection/state/AZ/AZ_data/AZ_transitions.csv')])

# OUTFLOWS TABLE
yearly_outflows_data = pd.read_csv('recidiviz/calculator/modeling/population_projection/state/LA/LA_data/Habitual Offender Prison Admissions Data-Table 1.csv')
...
for year in range(2009, 2019):
    temp_monthly_outflows_data = pd.DataFrame({
        ...
        'total_population': [yearly_outflows_data.iloc[(year - 2009) * 2, 4] / 12 for month in range(12)] +
                            [yearly_outflows_data.iloc[(year - 2009) * 2 + 1, 4] / 12 for month in range(12)]
        })
    outflows_data = pd.concat([outflows_data, temp_monthly_outflows_data], sort=False)

#TOTAL POPULATION TABLE
yearly_total_population_data = pd.read_csv('recidiviz/calculator/modeling/population_projection/state/LA/LA_data/Habitual Offender Total Population Data-Table 1.csv')
...
for year in range(2013, 2021):
    temp_monthly_total_population_data = pd.DataFrame({
        ...
        'crime_type': ['non-violent'] * 12 + ['violent'] * 12,
        'total_population': [yearly_total_population_data.iloc[(year - reference_year) * 2, 3] for month in range(12)] +
                            [yearly_total_population_data.iloc[(year - reference_year) * 2 + 1, 3] for month in range(12)]
    })
    total_population_data = pd.concat([total_population_data, temp_monthly_total_population_data], sort=False)

# STORE DATA
upload_spark_model_inputs('recidiviz-staging', 'LA_HB_364', outflows_data, transitions_data,
