...
for crime in crimes:
    crime_transitions_data = pd.DataFrame({
        ...
        'total_population': [100, 50 * mean(recidivism_rate_data[crime]), 50 * (1 - mean(recidivism_rate_data[crime]))],
...
# REMAINING_DURATION_DATA
remaining_duration_data = transitions_data.copy()
remaining_duration_data['remaining_duration'] = True

# STORE DATA
state = 'ex_state_microsim'
primary_compartment = 'prison'
a = pd.concat([transitions_data, outflows_data, total_population_data, remaining_duration_data]).to_csv(
