...
class Exporter:
    ...
    def upload_simulation_results_to_bq(
        ...
        if self.microsim:
            ...
            microsim_data = output_data['baseline_middle'].merge(
                output_data['baseline_min'], on=join_cols, suffixes=['', '_min']
            ).merge(output_data['baseline_max'], on=join_cols, suffixes=['', '_max'])

            microsim_data['year'] = \
                Initializer.convert_to_absolute_year(time_step, reference_year, microsim_data['time_step'])
            microsim_data = microsim_data.drop('time_step', axis=1)
            microsim_data = self._prep_for_upload(microsim_data, excluded_pop, total_pop)
            ignite_bq_utils.upload_ignite_results(project_id, simulation_tag, microsim_data)
            return {'microsim_data': microsim_data}

        spending_diff, compartment_life_years_diff, spending_diff_non_cumulative = self._get_output_metrics(
        ...
        aggregate_output_data = output_data['policy_simulation'].reset_index(drop=True).groupby(
            ['year', 'compartment']).sum().reset_index()
    ...
    def _get_output_metrics(
        ...
        cost_multipliers = \
            self._get_complete_cost_multipliers(cost_multipliers, sub_group_ids_dict, disaggregation_axes)
        ...
        for subgroup_tag, subgroup_dict in sub_group_ids_dict.items():
            ...
            for compartment_name, compartment_data in subgroup_data.groupby('compartment'):
                subgroup_life_years_diff.loc[compartment_data.year, compartment_name] = \
                    (compartment_data['control_total_population']
                     - compartment_data['policy_total_population']) * time_step

            subgroup_spending_diff_non_cumulative = subgroup_life_years_diff.copy() / time_step
            ...
            multiplier = cost_multipliers[(cost_multipliers[disaggregation_axes] ==
    ...
    def _get_complete_cost_multipliers(
        ...
        if cost_multipliers.empty:
            cost_multipliers = pd.DataFrame(columns=disaggregation_axes + ['multiplier'])

        missing_disaggregation_axes = [axis for axis in disaggregation_axes
        ...
        for subgroup_dict in sub_group_ids_dict.values():
            if cost_multipliers[(cost_multipliers[disaggregation_axes] ==
