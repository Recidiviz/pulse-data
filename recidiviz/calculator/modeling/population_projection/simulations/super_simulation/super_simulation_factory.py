...
class SuperSimulationFactory:
    ...
    def get_model_params(cls, yaml_file_path: str) -> Dict[str, Any]:
        ...
        model_params['reference_year'] = initialization_params.pop('reference_date', float)

        model_params['time_step'] = initialization_params.pop('time_step', float)

        model_params['disaggregation_axes'] = initialization_params.pop('disaggregation_axes', list)

        model_params['data_inputs_raw'] = cls._get_valid_data_inputs(initialization_params)
    ...
    def _get_valid_data_inputs(initialization_params: YAMLDict) -> Dict[str, Any]:
        ...
        if len(given_data_inputs) != 1:
            raise ValueError(f"Only one data input can be set in the yaml file, not {len(given_data_inputs)}")
        ...
        if 'big_query_inputs' in given_data_inputs.keys():
            ...
            data_inputs['big_query_simulation_tag'] = given_data_inputs.pop('big_query_simulation_tag', str)
        else:
            raise ValueError(f"Received unexpected key in data_inputs: {given_data_inputs.keys()[0]}")

        return data_inputs

    @staticmethod
    def _get_user_inputs(initialization_params: YAMLDict, model_params: Dict[str, Any]) -> Dict[str, Any]:
