...
from recidiviz.calculator.modeling.population_projection.simulations.population_simulation.\
    population_simulation import PopulationSimulation
...
class Validator:
    ...
    def calculate_outflows_error(self, simulation_title: str, outflows_data: pd.DataFrame) -> pd.DataFrame:
    ...
    def gen_total_population_error(self, simulation_tag: str, time_step: float, reference_year: float) -> pd.DataFrame:
        # Convert the index from relative time steps to floating point years
        error_results = self.pop_simulations[simulation_tag].gen_total_population_error()
        error_results.index = \
            Initializer.convert_to_absolute_year(time_step, reference_year, pd.Series(error_results.index))
        return error_results

    def gen_full_error_output(self, simulation_tag: str, time_step: float, reference_year: float) -> pd.DataFrame:
        error_results = self.pop_simulations[simulation_tag].gen_full_error()
        # Convert the index from relative time steps to floating point years
        error_results.index = error_results.index.set_levels(Initializer.convert_to_absolute_year(
            time_step, reference_year, pd.Series(error_results.index.levels[1])), level=1)
        ...
        for ts in range(len(error_differential.index)):
            for sub_group in error_differential.columns:
                error_differential.iloc[ts][sub_group] = \
                    cohort_population_error.iloc[ts + 1][sub_group] - cohort_population_error.iloc[ts][sub_group]
    ...
    def get_output_data_for_upload(self) -> Dict[str, Union[PopulationSimulation, pd.DataFrame]]:
        if self.microsim:
            return {simulation_title: self.pop_simulations[simulation_title].get_population_projections().sort_values(
                'time_step') for simulation_title in self.pop_simulations}
