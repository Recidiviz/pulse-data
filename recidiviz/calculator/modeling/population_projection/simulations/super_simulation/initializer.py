...
class Initializer:
    ...
    def __init__(self,
        ...
        self._initialize_data(data_inputs_params, compartments_architecture, disaggregation_axes)
    ...
    def get_outflows_for_error(self) -> pd.DataFrame:
        ...
        if self.microsim:
            return self.data_dict['outflows_data'][self.data_dict['outflows_data'].run_date ==
                                                   self.data_dict['outflows_data'].run_date.max()]
        return self.data_dict['outflows_data']

    def get_data_inputs(self, run_date_override: Optional[datetime] = None) -> Dict[str, Any]:
        ...
        if self.microsim:
            ...
            outflows = self._fully_connect_outflows(
                self.data_dict['outflows_data'][self.data_dict['outflows_data'].run_date == run_date])
            return {
    ...
    def get_inputs_for_calculate_prep_scale_factor(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def get_inputs_for_microsim_baseline_over_time(
        ...
        for start_date in start_run_dates:
            ...
            first_relevant_ts_dict[start_date] = self._convert_to_relative_date_from_timestamp(start_date)
    ...
    def _fully_connect_outflows(self, outflows_data: pd.DataFrame) -> pd.DataFrame:
        ...
        outflows_data = \
            outflows_data.groupby(fully_connected_columns)['total_population'].sum().unstack(level=['time_step'])
        ...
        return outflows_data.fillna(0).stack('time_step').reset_index(name='total_population')
    ...
    def _set_user_inputs(self, yaml_user_inputs: Dict[str, Any]) -> None:
        ...
        if self.microsim:
            ...
            self.user_inputs['policy_time_step'] = self._convert_to_relative_date(yaml_user_inputs['policy_year'])
    ...
    @staticmethod
    def convert_to_absolute_year(time_step: float, reference_year: float, time_steps: pd.Series) -> pd.Series:
    ...
    def _convert_to_absolute_year(self, time_steps: pd.Series) -> float:
        ...
        return self.convert_to_absolute_year(self.time_step, self.reference_year, time_steps)
    ...
    def _convert_to_relative_date_from_timestamp(self, timestamp: datetime) -> int:
        ...
        if not self.microsim:
            raise ValueError("Trying to use microsim method _convert_to_relative_date_from_timestamp() in macrosim")
