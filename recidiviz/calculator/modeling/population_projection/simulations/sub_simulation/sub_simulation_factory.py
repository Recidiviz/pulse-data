...
from recidiviz.calculator.modeling.population_projection.simulations.compartment_transitions \
    import CompartmentTransitions
...
class SubSimulationFactory:
    ...
    def build_sub_simulation(
        ...
        if not total_population_data.empty:
            if user_inputs['start_time_step'] not in total_population_data.time_step.values:
        ...
        transitions_per_compartment, shell_policies = \
            cls._initialize_transition_tables(transitions_data, compartments_architecture, policy_list)
        ...
        simulation_compartments = cls._build_compartments(historical_outflows, transitions_per_compartment,
                                                          shell_policies, user_inputs, compartments_architecture,
                                                          first_relevant_ts, total_population_data,
    ...
    def _initialize_transition_tables(cls, transitions_data: pd.DataFrame, compartments_architecture: Dict[str, str],
        ...
        for compartment in compartments_architecture:
            ...
            compartment_duration_data = transitions_data[transitions_data['compartment'] == compartment]
        ...
        for compartment in compartments_architecture:
            ...
            compartment_policies = SparkPolicy.get_compartment_policies(policy_list, compartment)
    ...
    @classmethod
    def _load_data(cls, compartments_architecture: Dict[str, str], outflows_data: pd.DataFrame) -> pd.DataFrame:
        ...
        preprocessed_data = outflows_data.groupby(['compartment', 'outflow_to', 'time_step'])[
    ...
    def _build_compartments(cls, preprocessed_data: pd.DataFrame,
        ...
        total_pop_data = total_population_data[total_population_data.time_step == user_inputs['start_time_step']]
            ...
            if should_initialize_compartment_populations:
                if isinstance(compartment_obj, FullCompartment):
                    compartment_population = \
                        total_population_data[total_population_data.compartment == compartment_tag]
                    if compartment_population.empty:
                        compartment_population = 0
                    elif len(compartment_population) > 1:
                        raise ValueError(f"Multiple total populations for the same compartment and time_step: \n"
