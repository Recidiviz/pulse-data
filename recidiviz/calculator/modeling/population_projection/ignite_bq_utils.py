...
def load_ignite_table_from_big_query(project_id: str, dataset: str, table_name: str, state_code: str):
...
def add_transition_rows(transition_data: pd.DataFrame):
    ...
    for run_date in transition_data.run_date.unique():
        extra_rows_release = pd.DataFrame({'compartment': ['RELEASE - RELEASE'] * 2,
        ...
        long_sentences = 1 - np.round(transition_data[transition_data.run_date == run_date].groupby([
            'compartment', 'gender']).sum().total_population, 6)
        ...
        extra_rows_long_sentence = pd.DataFrame({
            'compartment': long_sentences.index.get_level_values(level='compartment'),
            ...
            'compartment_duration': [48] * len(long_sentences)  # hard coded for now
        })
        complete_transitions = \
            pd.concat([complete_transitions, extra_rows_release, extra_rows_long_sentence.reset_index(drop=True)])
...
def upload_ignite_results(project_id, simulation_tag, microsim_population_df):
    ...
    store_simulation_results(project_id, MICROSIM_DATASET, MICROSIM_TABLE_NAME, MICROSIM_SCHEMA, microsim_population_df)
