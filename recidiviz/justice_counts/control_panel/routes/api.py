...
def get_api_blueprint(
    ...
    # Custom Child Agency Name
    @api_blueprint.route("/agency/<child_agency_id>/custom-name", methods=["PUT"])
    @auth_decorator
...
def _get_published_data(agency_id: int, is_v2: bool = False) -> Dict[str, Any]:
    ...
    if is_v2 is True:
        ...
        agency_json["population"] = AgencyJurisdictionInterface.get_agency_population(
            agency=agency, session=current_session
        )
    return {
        "agency": agency_json,
